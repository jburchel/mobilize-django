...
def church_detail(request, pk):
    ...
    memberships = (
        church.memberships.filter(status="active")
        .select_related("person__contact")
    ...
    try:
        ...
        recent_communications = (
            Communication.objects.filter(
                ...
                | models.Q(person__church_memberships__church=church)
            )
            .distinct()
    ...
    try:
        ...
        church_tasks = Task.objects.filter(church=church).order_by("-created_at")[:5]
    except ImportError:
        church_tasks = []
    ...
    if main_pipeline:
        ...
        try:
            pipeline_contact = PipelineContact.objects.get(
                contact=church.contact, pipeline=main_pipeline
            )
...
def church_delete(request, pk):
    ...
    return render(
        request,
        "churches/church_confirm_delete.html",
        {
            "church": church,
        },
    )
        ...
        if form.is_valid():
            ...
            try:
                ...
                if not decoded_file:
                    ...
                    return render(
                        request, "churches/import_churches.html", {"form": form}
                    )
                ...
                if "name" not in reader.fieldnames:
                    form.add_error(
                        "csv_file", 'Required column "name" not found in CSV file'
                    )
                    return render(
                        request, "churches/import_churches.html", {"form": form}
                    )
                        ...
                        if name:  # Only name is required
                            church, created = Church.objects.update_or_create(
                                ...
                                defaults={
                                    ...
                                    "congregation_size": (
                                        int(row.get("congregation_size", 0))
                                        if row.get("congregation_size", "").strip()
                                        else None
                                    ),
                                    "weekly_attendance": (
                                        int(row.get("weekly_attendance", 0))
                                        if row.get("weekly_attendance", "").strip()
                                        else None
                                    ),
                                    "church_pipeline": row.get(
                                        "church_pipeline", ""
                                    ).strip(),
                                    "priority": row.get("priority", "").strip(),
                                    "office_id": request.POST.get("office_id")
                                    or 1,  # Use office_id from form
                                },
                            )
...
def export_churches(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="churches_export_{timestamp}.csv"'
    )
    ...
    for church in churches:
        writer.writerow(
            [
                ...
                church.location or "",
                (
                    str(church.congregation_size)
                    if church.congregation_size is not None
                    else ""
                ),
                (
                    str(church.weekly_attendance)
                    if church.weekly_attendance is not None
                    else ""
                ),
                str(church.year_founded) if church.year_founded is not None else "",
                church.church_pipeline or "",
                church.priority
                or "",  # Use priority directly since get_priority_display() doesn't exist
                getattr(church, "notes", "") or "",
                (
                    getattr(church, "created_at", "").strftime("%Y-%m-%d %H:%M:%S")
                    if getattr(church, "created_at", None)
                    else ""
                ),
            ]
        )
        ...
        if person_id:
            ...
            try:
                ...
                if existing_membership:
                    messages.warning(
                        request,
                        f"{person.name} is already associated with this church.",
                    )
                else:
                    ...
                    messages.success(
                        request,
                        f"Successfully added {person.name} to {church.name} as {membership.get_role_display()}.",
                    )
...
def edit_church_member(request, membership_id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            f"Updated {membership.person.name}'s role in {membership.church.name}.",
        )
...
def set_primary_contact(request, membership_id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            f"Set {membership.person.name} as primary contact for {membership.church.name}.",
        )
...
def bulk_update_church_priority(request):
    ...
    try:
        ...
        messages.success(
            request,
            f"Successfully updated {count} church(es) to {priority_display} priority",
        )
...
def bulk_assign_church_user(request):
    ...
    try:
        ...
        messages.success(
            request, f"Successfully assigned {count} church(es) to {user_display_name}"
        )
...
def bulk_assign_church_office(request):
    ...
    try:
        ...
        messages.success(
            request, f"Successfully assigned {count} church(es) to {office.name}"
        )
