...
class ChurchForm(forms.ModelForm):
    ...
    pipeline_stage = forms.ChoiceField(
        choices=PIPELINE_CHOICES, required=False, label="Pipeline Stage"
    )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "service_times": forms.Textarea(
                attrs={
                    "rows": 2,
                    "placeholder": 'e.g., {"Sunday": "9am, 11am", "Wednesday": "7pm"}',
                }
            ),
            "facilities": forms.Textarea(
                attrs={
                    "rows": 2,
                    "placeholder": 'e.g., {"sanctuary_capacity": 500, "parking_spots": 100}',
                }
            ),
            "ministries": forms.Textarea(
                attrs={"rows": 2, "placeholder": 'e.g., ["Youth Group", "Food Pantry"]'}
            ),
            "secondary_languages": forms.Textarea(
                attrs={"rows": 2, "placeholder": 'e.g., ["Spanish", "Korean"]'}
            ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            Div(
                HTML(
                    '<div class="card mb-4"><div class="card-header bg-primary text-white"><h5 class="mb-0"><i class="fas fa-church me-2"></i>Basic Information</h5></div><div class="card-body">'
                ),
                Row(
            ...
            Div(
                HTML(
                    '<div class="card mb-4"><div class="card-header bg-success text-white"><h5 class="mb-0"><i class="fas fa-address-card me-2"></i>Contact Information</h5></div><div class="card-body">'
                ),
                Row(
            ...
            Div(
                HTML(
                    '<div class="card mb-4"><div class="card-header bg-warning text-dark"><h5 class="mb-0"><i class="fas fa-users me-2"></i>Leadership Information</h5></div><div class="card-body">'
                ),
            ...
            Div(
                HTML(
                    '<div class="card mb-4"><div class="card-header bg-info text-white"><h5 class="mb-0"><i class="fas fa-info-circle me-2"></i>Church Details</h5></div><div class="card-body">'
                ),
                Row(
            ...
            Div(
                HTML(
                    '<div class="card mb-4"><div class="card-header bg-secondary text-white"><h5 class="mb-0"><i class="fas fa-flag me-2"></i>Status & Pipeline</h5></div><div class="card-body">'
                ),
                Row(
                ...
                Row(
                    ...
                    Column(
                        "virtuous", css_class="col-md-6 mb-3 d-flex align-items-center"
                    ),
                ),
                HTML("</div></div>"),
            ),
            ...
            Div(
                HTML(
                    '<div class="card mb-4"><div class="card-header bg-dark text-white"><h5 class="mb-0"><i class="fas fa-sticky-note me-2"></i>Notes & Information</h5></div><div class="card-body">'
                ),
                "notes",
                "info_given",
                HTML("</div></div>"),
            ...
            Div(
                Submit(
                    "submit", "Save Church", css_class="btn btn-primary btn-lg me-3"
                ),
                HTML(
                    '<a href="{% url \'churches:church_list\' %}" class="btn btn-secondary btn-lg">Cancel</a>'
                ),
    ...
    def save(self, commit=True, user=None, office=None):
        ...
        if self.instance.pk:
            ...
            # Creating new church - ensure no ID is set on the instance
        ...
        if commit:
            ...
            # Auto-create Person records for people added to church form
            self._create_people_from_church_form(church, user, office)
    ...
    def _create_people_from_church_form(self, church, user=None, office=None):
        """
        Create Person records for people specified in the church form.
        This ensures that Senior Pastor, Missions Pastor, and Primary Contact
        are automatically added to the People list.
        """
        from mobilize.contacts.models import Person
        from .models import ChurchMembership

        # List of people to create from church form
        people_to_create = [
            {
                "first_name": self.cleaned_data.get("senior_pastor_first_name"),
                "last_name": self.cleaned_data.get("senior_pastor_last_name"),
                "phone": self.cleaned_data.get("pastor_phone"),
                "email": self.cleaned_data.get("pastor_email"),
                "role": "senior_pastor",
                "is_primary_contact": False,
            },
            {
                "first_name": self.cleaned_data.get("missions_pastor_first_name"),
                "last_name": self.cleaned_data.get("missions_pastor_last_name"),
                "phone": self.cleaned_data.get("mission_pastor_phone"),
                "email": self.cleaned_data.get("mission_pastor_email"),
                "role": "missions_pastor",
                "is_primary_contact": False,
            },
            {
                "first_name": self.cleaned_data.get("primary_contact_first_name"),
                "last_name": self.cleaned_data.get("primary_contact_last_name"),
        ...
        for person_data in people_to_create:
            # Skip if no name provided
            if not person_data["first_name"] and not person_data["last_name"]:
                continue
            ...
            if person_data["email"]:
                existing_person = Person.objects.filter(
                    contact__email=person_data["email"]
                ).first()

            if (
                not existing_person
                and person_data["first_name"]
                and person_data["last_name"]
            ):
                # Try to find by name if no email match
                existing_person = Person.objects.filter(
                    contact__first_name=person_data["first_name"],
                    contact__last_name=person_data["last_name"],
                ).first()

            if existing_person:
                # Person already exists, just create/update church membership
                membership, created = ChurchMembership.objects.get_or_create(
                    church=church,
                    person=existing_person,
                    defaults={
                        "role": person_data["role"],
                        "is_primary_contact": person_data["is_primary_contact"],
                        "status": "active",
                    },
                )
                if not created:
                    # Update existing membership
                    membership.role = person_data["role"]
                    membership.is_primary_contact = person_data["is_primary_contact"]
                    membership.save()
            else:
                # Create new person
                person_contact = Contact(
                    ...
                    email=person_data["email"] if person_data["email"] else None,
                    phone=person_data["phone"],
                    status="active",
                )
                if user:
                    person_contact.user = user
                if office:
                    person_contact.office = office

                # Handle potential duplicate email constraint error
                try:
                    person_contact.save()
                except Exception as e:
                    # If email conflict, try saving without email
                    if "email" in str(e).lower() and "unique" in str(e).lower():
                        person_contact.email = None
                        person_contact.save()
                    else:
                ...
                person = Person.objects.create(
                    contact=person_contact, primary_church=church
                )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            Div(
                ...
                HTML(
                    '<a href="{% url \'churches:church_list\' %}" class="btn btn-secondary ms-2">Cancel</a>'
                ),
                css_class="mt-4",
            ),
        )
