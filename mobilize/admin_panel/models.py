    ...
    def admin_count(self):
        ...
        try:
            ...
            return User.objects.filter(
                id__in=int_user_ids, role__in=["super_admin", "office_admin"]
            ).count()
...
class UserOffice(models.Model):
    ...
    user_id = models.CharField(
        max_length=50, db_column="user_id"
    )  # Explicitly define as CharField to match database
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_primary:
            ...
            UserOffice.objects.filter(user_id=self.user_id, is_primary=True).update(
                is_primary=False
            )
