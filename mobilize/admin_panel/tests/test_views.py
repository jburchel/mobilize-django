...
class AdminPanelViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.office = Office.objects.create(
            name="Test Office", code="TEST001", city="Test City"
        )

        # Assign office admin to office
        UserOffice.objects.create(
            user=self.office_admin, office=self.office, role="office_admin"
        )
...
class OfficeViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.office = Office.objects.create(
            name="Test Office", code="TEST001", city="Test City", phone="+1234567890"
        )
    ...
    def test_office_list_view(self):
        ...
        UserOffice.objects.create(
            user=self.admin_user, office=self.office, role="office_admin"
        )
...
class UserOfficeManagementTests(TestCase):
    ...
    def setUp(self):
        ...
        self.user_office = UserOffice.objects.create(
            user=self.regular_user, office=self.office, role="standard_user"
        )
    ...
    def test_add_user_to_office(self):
        ...
        new_user = User.objects.create_user(
            username="newuser", email="new@example.com", role="standard_user"
        )
        ...
        self.client.login(username="admin", password="adminpass123")
        url = reverse(
            "admin_panel:add_user_to_office", kwargs={"office_id": self.office.pk}
        )

        data = {"user_id": new_user.pk, "role": "standard_user"}
        ...
        user_office = UserOffice.objects.filter(
            user=new_user, office=self.office
        ).first()
    ...
    def test_remove_user_from_office(self):
        ...
        self.client.login(username="admin", password="adminpass123")
        url = reverse(
            "admin_panel:remove_user_from_office",
            kwargs={"office_id": self.office.pk, "user_id": self.regular_user.pk},
        )
        ...
        user_office_exists = UserOffice.objects.filter(
            user=self.regular_user, office=self.office
        ).exists()
    ...
    def test_change_user_role_in_office(self):
        ...
        self.client.login(username="admin", password="adminpass123")
        url = reverse(
            "admin_panel:update_user_office_role",
            kwargs={"office_id": self.office.pk, "user_id": self.regular_user.pk},
        )
