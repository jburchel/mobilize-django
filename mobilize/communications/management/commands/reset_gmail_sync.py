...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--user-id", type=int, help="Reset Gmail sync for specific user ID only"
        )
        parser.add_argument(
            "--days-back",
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING("Mode: DRY RUN (no changes will be made)")
            )
        ...
        if not skip_cleanup:
            ...
            gmail_emails = Communication.objects.filter(
                type__iexact="email", gmail_message_id__isnull=False
            )
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"Gmail sync failed: {str(e)}"))
            return
        ...
        if not dry_run:
            ...
            manual_comms = Communication.objects.filter(
                type__iexact="email", gmail_message_id__isnull=True
            )
            if user_id:
