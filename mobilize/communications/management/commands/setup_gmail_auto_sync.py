    ...
    def handle(self, *args, **options):
        ...
        if disable:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✅ Disabled automatic Gmail sync task: {task_name}"
                    )
                )
            except PeriodicTask.DoesNotExist:
                self.stdout.write(
                    self.style.WARNING(
                        f"⚠️  No automatic Gmail sync task found to disable"
                    )
                )
            return
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(
                    f"✅ Created new interval schedule: every {interval_minutes} minutes"
                )
            )
        ...
        self.stdout.write(
            "   1. Make sure Celery worker is running: ./start_celery.sh worker"
        )
        self.stdout.write(
            "   2. Make sure Celery beat scheduler is running: ./start_celery.sh beat"
        )
        ...
        self.stdout.write(
            '   >>> print(f"Last run: {task.last_run_at}, Enabled: {task.enabled}")'
        )
