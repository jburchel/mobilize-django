...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING("DRY RUN MODE - Analysis only"))
        ...
        if not churches.exists():
            self.stdout.write(self.style.ERROR("No churches found matching criteria"))
            return
        ...
        for church in churches:
            ...
            current_comms = (
                ...
                .filter(
                    ...
                    | models.Q(sender__icontains=church.contact.email)
                    if church.contact.email
                    else models.Q(pk=None)
                )
                .order_by("-date")
            )

            self.stdout.write(
                f"   Current communications (past year): {current_comms.count()}"
            )

            if verbose and current_comms.exists():
                for comm in current_comms[:5]:
                    self.stdout.write(
                        f"     • {comm.date}: {comm.subject} (from {comm.sender})"
                    )
            ...
            if church.contact.email:
                ...
                self.stdout.write(f"   🔄 To sync Gmail for this church, run:")
                self.stdout.write(
                ...
                email_pattern_comms = Communication.objects.filter(
                    date__gte=one_year_ago, sender__icontains=church.contact.email
                ).count()
                ...
                self.stdout.write(
                    self.style.WARNING("   ⚠️ No email address - cannot sync Gmail")
                )
            ...
            for part in name_parts:
                if len(part) > 3:  # Skip short words
                    similar_comms = Communication.objects.filter(
                        date__gte=one_year_ago, sender__icontains=part
                    ).exclude(
                        sender__icontains=(
                            church.contact.email if church.contact.email else ""
                        )
                    )
