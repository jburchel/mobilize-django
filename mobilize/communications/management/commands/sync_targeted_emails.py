...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
            ...
            if not contact_found:
                for name_part in target_name.split():
                    if len(name_part) > 2:  # Skip short words
                        contact = Contact.objects.filter(
                            Q(first_name__icontains=name_part) | Q(last_name__icontains=name_part),
                            type='person'
                        ).first()
        ...
        for contact, target_name in found_contacts:
            full_name = f"{contact.first_name or ''} {contact.last_name or ''}".strip()
            self.stdout.write(f'  ✓ {target_name} -> {full_name} ({contact.email or "no email"})')
        ...
        if not sync_user:
            self.stdout.write(self.style.ERROR('No user found for Gmail sync. Use --user-email to specify.'))
            return
        ...
        self.stdout.write(f'Syncing last {days_back} days of emails using user: {sync_user.email}')
