...
class Command(BaseCommand):
    help = (
        "Clean up Gmail-synced emails while preserving manually logged communications"
    )

    def add_arguments(self, parser):
        parser.add_argument(
            "--user-id", type=int, help="Clean up emails for specific user ID only"
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        user_counts = (
            queryset.values("user_id").annotate(count=Count("id")).order_by("-count")
        )
        ...
        if dry_run:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Would delete {total_count} Gmail-synced communications."
                )
            )
            return
        ...
        try:
            with transaction.atomic():
                ...
                if not table_exists:
                    self.stdout.write(
                        self.style.WARNING(
                            "EmailAttachment table does not exist, proceeding with direct deletion..."
                        )
                    )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully deleted {deleted_count} communications."
                )
            )
