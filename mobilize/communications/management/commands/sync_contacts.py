...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--user-id',
            type=int,
            help='Sync contacts for specific user ID only'
        )
        parser.add_argument(
            '--force',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(
                self.style.SUCCESS('[DRY RUN] Google Contacts sync')
            )
        else:
            self.stdout.write(
                self.style.SUCCESS('Starting Google Contacts sync...')
            )
        ...
        if user_id:
            ...
            if not force_sync:
                ...
                user_ids_to_sync = [
                    setting.user_id for setting in sync_settings 
        ...
        for user in users:
            try:
                sync_settings = UserContactSyncSettings.objects.filter(user=user).first()
                
                if not sync_settings or sync_settings.sync_preference == 'disabled':
                    self.stdout.write(
                        self.style.WARNING(f'User {user.username} ({user.id}) - Contact sync disabled')
                    )
                    continue
                ...
                if not contacts_service.is_authenticated():
                    self.stdout.write(
                        self.style.WARNING(f'User {user.username} ({user.id}) - Google Contacts not authenticated')
                    )
                    continue
                ...
                if dry_run:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f'User {user.username} ({user.id}) - Ready for sync: {mode}')
                    )
                    users_processed += 1
                else:
                    ...
                    if result['success']:
                        ...
                        if 'errors' in result and result['errors']:
                            ...
                            if len(result['errors']) > 3:
                                self.stdout.write(
                                    self.style.WARNING(f'  ... and {len(result["errors"]) - 3} more warnings')
                                )
                    else:
                        self.stdout.write(
                            self.style.ERROR(
        ...
        if dry_run:
            self.stdout.write(
                self.style.SUCCESS(f'[DRY RUN] {users_processed} users ready for contact sync')
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(
