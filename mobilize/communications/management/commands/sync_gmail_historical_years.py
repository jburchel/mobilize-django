...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Chunk size: {chunk_months} months, Max per chunk: {max_per_chunk}')
        ...
        if user_id:
            ...
            try:
                ...
                if users.count() == 1:
                    ...
                    self.stdout.write(self.style.ERROR('Multiple users found. Please specify --user-id'))
                    return
            except Exception:
                self.stdout.write(self.style.ERROR('Could not find Gmail authenticated user'))
                return
        ...
        if not gmail_service.is_authenticated():
            self.stdout.write(self.style.ERROR(f'User {user.username} - Gmail not authenticated'))
            return
        
        if dry_run:
            self.stdout.write(self.style.SUCCESS('User ready for multi-year historical sync'))
            return
    ...
    def sync_date_chunks(self, gmail_service, user, known_emails, existing_message_ids, start_date, end_date, chunk_months, max_per_chunk, delay):
        ...
        while current_date < end_date:
            ...
            self.stdout.write(f'Chunk {chunk_num} synced: {chunk_synced} emails (total: {total_synced})')
    ...
    def sync_date_range(self, gmail_service, user, known_emails, existing_message_ids, start_date, end_date, max_per_chunk):
        ...
        for folder in ['inbox', 'sent']:
            try:
                ...
                messages = gmail_service.get_messages(query=query, max_results=max_per_chunk)
