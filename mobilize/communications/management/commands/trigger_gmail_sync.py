...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--user-id", type=int, help="Sync Gmail for specific user ID"
        )
        parser.add_argument(
            "--username", type=str, help="Sync Gmail for specific username"
        )
        parser.add_argument(
            "--all-users",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        if options["all_users"]:
            ...
            if run_async:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✓ Queued Gmail sync for all users (Task ID: {task.id})"
                    )
                )
            else:
            ...
            try:
                ...
                if run_async:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"✓ Queued Gmail sync for {user.username} (Task ID: {task.id})"
                        )
                    )
                else:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"✓ Gmail sync completed for {user.username}: {result}"
                        )
                    )
