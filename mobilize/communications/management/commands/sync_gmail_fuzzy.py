...
class Command(BaseCommand):
    help = (
        "Sync Gmail emails using fuzzy name matching when email addresses don't match"
    )
    ...
    def find_contact_by_name_or_email(self, name, email):
        ...
        if name:
            ...
            church = Church.objects.filter(
                Q(name__icontains=name) | Q(contact__church_name__icontains=name)
            ).first()
    ...
    def handle(self, *args, **options):
        ...
        if user_id:
            ...
            self.stdout.write(
                f"Found {users.count()} authenticated users: {[u.username for u in users]}"
            )
        ...
        for user in users:
            ...
            if not gmail_service.is_authenticated():
                self.stdout.write(
                    self.style.WARNING(f"Gmail not authenticated for {user.username}")
                )
            ...
            self.stdout.write(
                f"Found {len(messages)} emails in the last {days_back} days"
            )
            ...
            if messages:
                ...
                self.stdout.write(
                    f"Found {len(broader_messages)} emails without date filter"
                )
                        ...
                        try:
                            ...
                            if person:
                                ...
                                with connection.cursor() as cursor:
                                    cursor.execute(
                                        "SELECT id FROM people WHERE contact_id = %s",
                                        [person.pk],
                                    )
