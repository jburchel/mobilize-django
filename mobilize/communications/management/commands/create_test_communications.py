...
class Command(BaseCommand):
    ...
    def cleanup_test_communications(self):
        ...
        if count == 0:
            self.stdout.write(self.style.WARNING('No test communications found to cleanup'))
            return
        ...
        if confirm.lower() in ['y', 'yes']:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Successfully deleted {deleted_count} test communications')
            )
        else:
