        ...
        for email_addr in target_emails:
            ...
            if church_contacts.exists():
                for contact in church_contacts:
                    self.stdout.write(
                        f"   🏛️ Church: {contact.church_name} (Contact ID: {contact.id})"
                    )

            # COMPREHENSIVE SEARCH - Check ALL possible ways this email could appear
            email_communications = (
                Communication.objects.filter(date__gte=start_date)
                .filter(
            ...
            for comm in partial_communications:
                if comm.id not in all_comm_ids:
                    ...
                    if (
                        email_addr.lower() in (comm.sender or "").lower()
                        or email_addr.lower() in (comm.message or "").lower()
                        or local_part.lower() in (comm.sender or "").lower()
                    ):
                        all_comm_ids.add(comm.id)
                        email_related_comms.append(comm)
            ...
            # Sort by date
            email_related_comms.sort(
                key=lambda x: x.date or timezone.now().date(), reverse=True
            )

            self.stdout.write(
                f"   📊 TOTAL COMMUNICATIONS FOUND: {len(email_related_comms)}"
            )
            ...
            if email_related_comms:
                ...
                for i, comm in enumerate(email_related_comms, 1):
                    ...
                    self.stdout.write(
                        f"     {i:2d}. {comm.date}: [{comm_type}] {direction}"
                    )
                ...
                self.stdout.write(
                    f"   💡 This might indicate the emails have not been synced yet"
                )
