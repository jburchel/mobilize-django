...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if user_email:
            ...
            user_ids = Communication.objects.filter(
                gmail_message_id__isnull=False
            ).exclude(gmail_message_id='').values_list('user_id', flat=True).distinct()
        ...
        for user in users:
            ...
            if not gmail_service.service:
                self.stdout.write(self.style.WARNING(f'  Gmail not authenticated for {user.email}, skipping'))
                continue
            ...
            for comm in communications:
                try:
                    ...
                    message_detail = gmail_service.service.users().messages().get(
                        userId='me',
                        id=comm.gmail_message_id
                    ).execute()
                    ...
                    if email_date_str:
                        try:
                            ...
                            self.stdout.write(
                                self.style.WARNING(f'    ✗ Error parsing date "{email_date_str}": {e}')
                            )
                            error_count += 1
                    else:
                        self.stdout.write(
                            self.style.WARNING(f'    ✗ No date found for: {comm.subject[:50]}...')
                        )
                        error_count += 1
                    ...
                    self.stdout.write(
                        self.style.ERROR(f'    ✗ Error fetching message {comm.gmail_message_id}: {e}')
                    )
                    error_count += 1
                    total_processed += 1
            
            self.stdout.write(f'  User {user.email}: Updated {updated_count}, Errors {error_count}')
