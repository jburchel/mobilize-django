...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--email',
            type=str,
            help='Specific email address to sync'
        )
        parser.add_argument(
            '--all-target-emails',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Starting targeted email sync for {len(emails_to_sync)} addresses...')
        )
        ...
        for email_addr in emails_to_sync:
            ...
            if people.exists():
                ...
                for person in people:
                    ...
                    self.stdout.write(f'        Current communications: {current_comms}')
            else:
                ...
                if church_contacts.exists():
                    self.stdout.write(f'   Found {church_contacts.count()} church contacts with this email:')
                    for contact in church_contacts:
                        self.stdout.write(f'     🏛️ {contact.church_name} (Contact ID: {contact.id})')
            ...
            self.stdout.write(f'   Existing communications FROM this email: {existing_sender_comms}')
            ...
            if not dry_run:
                ...
                self.stdout.write(f'      python manage.py sync_gmail --days-back {days_back} --all-emails')
            else:
