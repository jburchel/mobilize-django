...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for email_addr in target_emails:
            ...
            church_contact = Contact.objects.filter(
            ...
            if person:
                ...
                self.stdout.write(
                    f"   🏛️ Church: {church_contact.church_name} (Contact ID: {church_contact.id})"
                )
            else:
                self.stdout.write(
                    f"   ❌ No person or church contact found with this email"
                )
                continue
            ...
            from_comms = Communication.objects.filter(
                date__gte=one_year_ago, sender__icontains=email_addr
            ).order_by("-date")

            # 2. Communications TO this email (in message content, CC, BCC)
            to_comms = (
                Communication.objects.filter(date__gte=one_year_ago)
                .filter(
            ...
            # Sort by date
            all_related_comms.sort(
                key=lambda x: x.date or timezone.now().date(), reverse=True
            )

            self.stdout.write(
                f"   📊 Total related communications found: {len(all_related_comms)}"
            )
            ...
            if all_related_comms:
                ...
                for comm in all_related_comms:
                    ...
                    if verbose:
                        self.stdout.write(
                            f"     • {comm.date}: [{comm_type}] {direction} - {subject}"
                        )
                ...
                if domain:
                    domain_comms = Communication.objects.filter(
                        date__gte=one_year_ago, sender__icontains=domain
                    ).count()

                    if domain_comms > 0:
                        self.stdout.write(
                            f"   💡 Found {domain_comms} communications from domain {domain}"
                        )
