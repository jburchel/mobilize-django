...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(
                self.style.SUCCESS('[DRY RUN] Gmail comprehensive historical sync (contacts only)')
            )
        else:
            self.stdout.write(
                self.style.SUCCESS('Starting Gmail comprehensive historical sync (contacts only)...')
            )
            self.stdout.write(
        ...
        for user in users:
            try:
                ...
                if not gmail_service.is_authenticated():
                    self.stdout.write(
                        self.style.WARNING(f'User {user.username} - Gmail not authenticated, skipping')
                    )
                    continue
                
                if dry_run:
                    # For dry run, estimate emails to sync
                    self.stdout.write(f'User {user.username} - Ready for full historical sync')
                    users_processed += 1
                    continue
                ...
                self.stdout.write(f'Found {len(existing_message_ids)} already synced emails for this user')
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'User {user.username} - Synced {user_synced} new emails')
                )
    ...
    def sync_user_full_history(self, gmail_service, user, existing_message_ids, batch_size, max_results, delay):
        ...
        for folder in ['inbox', 'sent']:
            ...
            try:
                ...
                try:
                    all_messages = gmail_service.get_messages(query=query, max_results=max_results)
                    ...
                    for i in range(0, len(all_messages), batch_size):
                        ...
                        try:
                            ...
                            self.stdout.write(f'  Error processing batch {i//batch_size + 1}: {batch_error}')
    ...
    def process_message_batch(self, gmail_service, user, messages, known_emails, existing_message_ids, folder):
        ...
        for message in messages:
            try:
                ...
                if self.should_sync_message(message, user, known_emails, folder):
                    ...
                    if self.sync_single_message(gmail_service, user, message, known_emails):
                        batch_synced += 1
                        existing_message_ids.add(message['id'])  # Track to avoid duplicates
                        
            except Exception as e:
                self.stdout.write(f"Error processing message {message.get('id', 'unknown')}: {e}")
                continue
