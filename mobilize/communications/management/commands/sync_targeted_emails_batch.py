...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if missing_contacts:
            self.stdout.write(
                self.style.WARNING(
                    f'Missing {len(missing_contacts)} contacts: {", ".join(missing_contacts)}'
                )
            )
        ...
        # Create batches
        batches = [
            contact_emails[i : i + batch_size]
            for i in range(0, len(contact_emails), batch_size)
        ]

        self.stdout.write(
            f"Processing {len(contact_emails)} emails in {len(batches)} batches of {batch_size}"
        )

        if dry_run:
            self.stdout.write(
                self.style.WARNING("DRY RUN - Would process the following batches:")
            )
            for i, batch in enumerate(batches, 1):
                self.stdout.write(
                    f'  Batch {i}: {", ".join([name for _, name in batch])}'
                )
            return
        ...
        for batch_num, batch in enumerate(batches, 1):
            ...
            try:
                ...
                if result["success"]:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"✓ Batch {batch_num}: Synced {batch_count} emails"
                        )
                    )
                else:
                    self.stdout.write(
                        self.style.ERROR(
                            f'✗ Batch {batch_num}: {result.get("error", "Unknown error")}'
                        )
                    )

            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f"✗ Batch {batch_num}: Exception - {e}")
                )
        ...
        self.stdout.write(
            f"Processed {len(batches)} batches for {len(contact_emails)} contacts"
        )
