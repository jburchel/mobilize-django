    ...
    try:
        ...
        for communication in pending_communications:
            try:
                ...
                processed_count += 1
                logger.info(
                    f"Queued email communication {communication.id} for sending"
                )

            except Exception as e:
                failed_count += 1
                logger.error(
                    f"Failed to queue email communication {communication.id}: {str(e)}"
                )
...
def send_email_communication(self, communication_id: int):
    ...
    try:
        ...
        if not to_email:
            raise ValueError(
                f"No email address for contact {communication.person.contact.id}"
            )

        # Get email signature if available
        signature = ""
        try:
            email_signature = EmailSignature.objects.get(
                user=communication.user, is_default=True
            )
        ...
        message = gmail_service.send_email(
            ...
            body=content,
            cc=(
                communication.cc_recipients.split(",")
                if communication.cc_recipients
                else None
            ),
            bcc=(
                communication.bcc_recipients.split(",")
                if communication.bcc_recipients
                else None
            ),
        )
        ...
        logger.error(
            f"Failed to send email communication {communication_id}: {str(exc)}"
        )
        raise self.retry(exc=exc)
...
def sync_gmail_emails(
    self, user_id: int, days_back: int = 7, specific_emails: list = None
):
    """
    ...
    try:
        ...
        result = gmail_service.sync_emails_to_communications(
            days_back, contacts_only=contacts_only, specific_emails=specific_emails
        )
...
def sync_all_users_gmail(self, days_back: int = 1):
    ...
    try:
        ...
        authenticated_users = User.objects.filter(
            id__in=GoogleToken.objects.filter(access_token__isnull=False).values_list(
                "user_id", flat=True
            ),
            is_active=True,
        )
        ...
        for user, sync_settings in users_to_sync:
            try:
                ...
                if result.get("status") == "success":
                    ...
                    logger.info(
                        f"Skipped user {user.username} - {result.get('reason')}"
                    )
                    sync_settings.update_last_sync(
                        error_message=f"Skipped: {result.get('reason')}"
                    )
                else:
                    error_msg = result.get("reason", "Unknown error")
                    logger.warning(
                        f"Gmail sync failed for user {user.username}: {error_msg}"
                    )
                ...
                error_msg = str(e)
                logger.error(
                    f"Error syncing Gmail for user {user.username}: {error_msg}"
                )
                sync_settings.update_last_sync(error_message=error_msg)
        ...
        logger.info(
            f"Gmail auto-sync completed: {total_synced} emails synced for {users_processed} users"
        )
        return {
    ...
    try:
        ...
        for user_data in users_with_new_emails:
            try:
                ...
                if skipped_count > 0:
                    message += (
                        f" ({skipped_count} emails from unknown contacts were skipped.)"
                    )
    ...
    try:
        ...
        for contact in contacts:
            try:
                ...
                failed_count += 1
                logger.error(
                    f"Failed to create communication for contact {contact.id}: {str(e)}"
                )

        logger.info(
            f"Bulk email job completed: {sent_count} queued, {failed_count} failed"
        )
        return {
...
def cleanup_old_communications(self, days_to_keep: int = 90):
    ...
    try:
        ...
        deleted_count = Communication.objects.filter(
            created_at__lt=cutoff_date, status__in=["sent", "delivered"]
        ).delete()[0]
...
def generate_email_analytics(self, user_id: int, date_range_days: int = 30):
    ...
    try:
        ...
        communications = Communication.objects.filter(
            user=user, type="email", created_at__gte=start_date
        )
        ...
        cache.set(
            f"email_analytics_{user_id}", analytics, timeout=3600
        )  # Cache for 1 hour
