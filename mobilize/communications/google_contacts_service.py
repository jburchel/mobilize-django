...
class GoogleContactsService:
    ...
    def get_all_contacts(self, page_size: int = 1000) -> List[Dict]:
        ...
        try:
            ...
            while True:
                ...
                results = (
                    self.service.people().connections().list(**request_params).execute()
                )
    ...
    def parse_contact(self, contact_data: Dict) -> Dict:
        ...
        if emails:
            primary_email = next(
                (e for e in emails if e.get("metadata", {}).get("primary")), emails[0]
            )
        ...
        if phones:
            primary_phone = next(
                (p for p in phones if p.get("metadata", {}).get("primary")), phones[0]
            )
    ...
    def sync_contacts_based_on_preference(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not sync_settings or sync_settings.sync_preference == "disabled":
                return {
                    "success": True,
                    "message": "Contact sync is disabled",
                    "synced_count": 0,
                }
    ...
    def _sync_crm_contacts_only(self) -> Dict[str, Any]:
        ...
        existing_contacts = Contact.objects.filter(email__isnull=False).exclude(
            email=""
        )
        existing_emails = {
            contact.email.lower(): contact for contact in existing_contacts
        }
        ...
        for google_contact in google_contacts:
            try:
                ...
                errors.append(
                    f"Error syncing contact {parsed.get('email', 'unknown')}: {str(e)}"
                )
    ...
    def _sync_all_contacts_to_crm(self) -> Dict[str, Any]:
        ...
        for google_contact in google_contacts:
            try:
                ...
                errors.append(
                    f"Error importing contact {parsed.get('email', 'unknown')}: {str(e)}"
                )
    ...
    def get_all_contacts_for_selection(self) -> List[Dict]:
        ...
        for contact_data in google_contacts:
            ...
            formatted_contact = {
                ...
                or f"{parsed['first_name']} {parsed['last_name']}".strip()
                or "No Name",
                "email": parsed["email"],
    ...
    def import_selected_contacts(
        ...
        if not self.service or not selected_contact_ids:
            return {
                "success": False,
                "error": "No contacts selected or service unavailable",
            }
        ...
        try:
            ...
            selected_contacts = [
                contacts_to_import[contact_id]
                for contact_id in selected_contact_ids
                if contact_id in contacts_to_import
            ]
            ...
            for contact in selected_contacts:
                try:
                    ...
                    if not email:
                        errors.append(
                            f"Skipped contact '{contact['name']}' - no email address"
                        )
                        continue
                    ...
                    if existing_contact:
                        # Update existing contact
                        updated = self._update_contact_from_google(
                            existing_contact, google_data
                        )
                        if updated:
                            updated_count += 1
                    else:
                        ...
                        try:
                            ...
                            errors.append(
                                f"Failed to create contact '{contact['name']}': {str(create_error)}"
                            )
                            import logging

                            logger = logging.getLogger(__name__)
                            logger.error(
                                f"Failed to create contact {email}: {str(create_error)}",
                                exc_info=True,
                            )
