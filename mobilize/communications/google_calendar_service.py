...
class GoogleCalendarService:
    ...
    def sync_events_to_tasks(self, days_ahead: int = 30) -> Dict[str, Any]:
        ...
        events = self.get_events(
            calendar_id="primary", time_min=time_min, time_max=time_max, max_results=100
        )
        ...
        for event in events:
            ...
            if not Task.objects.filter(google_calendar_event_id=event["id"]).exists():
    ...
    def create_event_from_task(
        self, task, calendar_id: str = "primary"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if task.due_time:
                ...
                if isinstance(task.due_time, str):
                    try:
                        ...
                        due_time_obj = dt.strptime(
                            "09:00", "%H:%M"
                        ).time()  # Default to 9 AM
                else:
            ...
            return {
                "success": False,
                "error": f"Error creating calendar event from task: {error}",
            }
    ...
    def _convert_task_recurrence_to_rrule(self, task) -> List[str]:
        ...
        try:
            ...
            if frequency == "weekly" and pattern.get("weekdays"):
                ...
                weekdays = [
                    day_map[day] for day in pattern["weekdays"] if day in day_map
                ]
                if weekdays:
