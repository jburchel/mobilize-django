...
class LargeDatasetPerformanceTest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.office = Office.objects.create(
            name="Performance Test Office", code="PERF", is_active=True
        )
        ...
        self.pipeline = Pipeline.objects.create(
            name="Performance Pipeline", office=self.office, pipeline_type="person"
        )
    ...
    def test_contact_list_performance_with_1000_records(self):
        ...
        print(
            f"Contact list view loaded in {elapsed_time:.2f} seconds for 1000 records"
        )
    ...
    def test_contact_search_performance(self):
        ...
        for search_term in search_terms:
            ...
            response = self.client.get(
                reverse("contacts:person_list"), {"search": search_term}
            )
            elapsed_time = time.time() - start_time
            ...
            self.assertLess(
                elapsed_time, 1.0, f"Search for '{search_term}' took too long"
            )
    ...
    def test_communication_history_performance(self):
        ...
        self.assertEqual(response.status_code, 200)
        print(
            f"Communication list loaded in {elapsed_time:.2f} seconds for 5000 records"
        )
    ...
    def test_pipeline_visualization_performance(self):
        ...
        for i, person in enumerate(people):
            ...
            pc = PipelineContact(
                contact=person.contact, pipeline=self.pipeline, current_stage=stage
            )
        ...
        start_time = time.time()
        response = self.client.get(
            reverse("pipeline:pipeline_visualization", args=[self.pipeline.id])
        )
        elapsed_time = time.time() - start_time
    ...
    def test_pagination_performance(self):
        ...
        for page in page_numbers:
            start_time = time.time()
            response = self.client.get(reverse("contacts:person_list"), {"page": page})
            elapsed_time = time.time() - start_time
    ...
    def test_concurrent_user_performance(self):
        ...
        print(
            f"5 concurrent dashboard requests completed in {elapsed_time:.2f} seconds"
        )
...
class DatabaseIndexPerformanceTest(TestCase):
    ...
    def setUp(self):
        self.office = Office.objects.create(
            name="Index Test Office", code="INDEX", is_active=True
        )

        self.user = User.objects.create_user(
            username="indexuser", email="index@example.com", password="indexpass123"
        )
    ...
    def test_email_lookup_performance(self):
        ...
        self.assertLess(
            elapsed_time, 0.01, "Email lookup too slow - index may be missing"
        )
    ...
    def test_foreign_key_join_performance(self):
        ...
        tasks_with_contacts = (
            Task.objects.select_related("person__contact")
            .filter(status="pending", person__contact__office=self.office)
            .count()
        )
        ...
        print(
            f"Join query found {tasks_with_contacts} tasks in {elapsed_time:.4f} seconds"
        )
