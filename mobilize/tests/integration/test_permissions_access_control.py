...
class UserRolePermissionTests(TestCase):
    ...
    def setUp(self):
        ...
        self.office1 = Office.objects.create(
            name="Office 1", code="OFF1", is_active=True
        )

        self.office2 = Office.objects.create(
            name="Office 2", code="OFF2", is_active=True
        )
    ...
    def test_super_admin_access_all_offices(self):
        ...
        test_contacts = Contact.objects.filter(
            email__in=["office1test@example.com", "office2test@example.com"]
        )
...
class OfficeDataSegregationTests(TestCase):
    ...
    def setUp(self):
        ...
        self.office_alpha = Office.objects.create(
            name="Alpha Office", code="ALPHA", is_active=True
        )

        self.office_beta = Office.objects.create(
            name="Beta Office", code="BETA", is_active=True
        )
    ...
    def test_office_contact_isolation(self):
        ...
        response = self.client.get(
            reverse("contacts:person_detail", args=[alpha_person.pk])
        )
        self.assertEqual(response.status_code, 200)

        # Should NOT see beta office contact
        response = self.client.get(
            reverse("contacts:person_detail", args=[beta_person.pk])
        )
    ...
    def test_office_communication_isolation(self):
        ...
        response = self.client.get(
            reverse("communications:communication_detail", args=[beta_comm.id])
        )
        self.assertIn(response.status_code, [403, 404])
...
class APIPermissionTests(TestCase):
    ...
    def setUp(self):
        self.office = Office.objects.create(
            name="API Test Office", code="API", is_active=True
        )
    ...
    def test_authentication_required_for_api(self):
        ...
        )  # Permission denied - no office assignment
    ...
    def test_csrf_protection(self):
        ...
        response = self.client.post(
            ...
            {"first_name": "CSRF", "last_name": "Test", "email": "csrf@example.com"},
        )
    ...
    def test_user_settings_privacy(self):
        ...
        user_settings = UserContactSyncSettings.objects.create(
            user=self.authenticated_user, sync_preference="crm_only"
        )

        other_settings = UserContactSyncSettings.objects.create(
            user=other_user, sync_preference="all_contacts"
        )
        ...
        own_settings = UserContactSyncSettings.objects.filter(
            user=self.authenticated_user
        ).first()
...
class SessionSecurityTests(TestCase):
    ...
    def setUp(self):
        self.office = Office.objects.create(
            name="Session Test Office", code="SESSION", is_active=True
        )

        self.user = User.objects.create_user(
            username="sessionuser", email="session@example.com", password="testpass123"
        )
...
class EmailSignaturePermissionTests(TestCase):
    ...
    def setUp(self):
        self.office = Office.objects.create(
            name="Email Test Office", code="EMAIL", is_active=True
        )

        self.user1 = User.objects.create_user(
            username="emailuser1", email="email1@example.com", password="testpass123"
        )

        self.user2 = User.objects.create_user(
            username="emailuser2", email="email2@example.com", password="testpass123"
        )
    ...
    def test_email_signature_ownership(self):
        ...
        signature1 = EmailSignature.objects.create(
            user=self.user1, name="User1 Signature", content="Best regards,\nUser 1"
        )

        # User2 creates signature
        signature2 = EmailSignature.objects.create(
            user=self.user2, name="User2 Signature", content="Best regards,\nUser 2"
        )
        ...
        response = self.client.get(
            reverse("communications:email_signature_update", args=[signature1.id])
        )
        self.assertEqual(response.status_code, 200)

        # User1 should NOT be able to edit User2's signature
        response = self.client.get(
            reverse("communications:email_signature_update", args=[signature2.id])
        )
        self.assertIn(response.status_code, [403, 404])
