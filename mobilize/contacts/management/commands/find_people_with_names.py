...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('=== Finding People with Names in People Table ==='))
        ...
        try:
            ...
            if people_with_names:
                self.stdout.write(f'Found {len(people_with_names)} people with names in people table:')
                ...
                for person_id, p_first, p_last, contact_id in people_with_names:
                    ...
                    if contact_id:
                        ...
                        if contact_data:
                            ...
                            self.stdout.write(f'  Contact: "{c_first or ""}" "{c_last or ""}" ({contact_type})')
                            ...
                            if (p_first != c_first) or (p_last != c_last):
                                ...
                                if django_contact:
                                    ...
                                    if not dj_first and not dj_last:
                                        self.stdout.write(f'  → This would show as "No name listed" in Django!')
                            else:
                ...
                if mismatches:
                    ...
                    for contact_id, p_first, p_last, c_first, c_last, email in mismatches:
                        ...
                        if options['fix']:
                            if options['dry_run']:
                                self.stdout.write(f'  → WOULD UPDATE contact to: "{p_first}" "{p_last}"')
                            else:
                                cursor.execute("""
                                    UPDATE contacts 
                                ...
                                self.stdout.write(f'  → UPDATED contact to: "{p_first}" "{p_last}"')
        ...
        try:
            ...
            if no_name_people:
                self.stdout.write(f'Found {len(no_name_people)} people with no names in contacts:')
                
                for contact_id, c_first, c_last, email, p_first, p_last in no_name_people:
                    self.stdout.write(f'\nContact {contact_id} ({email}):')
                    self.stdout.write(f'  Contact names: "{c_first or ""}" "{c_last or ""}"')
                    self.stdout.write(f'  People names: "{p_first or ""}" "{p_last or ""}"')
