...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Found {total_empty} people with missing names in contacts table')
        ...
        for i, contact in enumerate(empty_name_contacts[:20]):
            try:
                ...
                if contact.first_name and not contact.last_name:
                    potential_names.append(f'partial: first_name only "{contact.first_name}"')
                    recovery_sources['partial_names'] += 1
                elif contact.last_name and not contact.first_name:
                    potential_names.append(f'partial: last_name only "{contact.last_name}"')
                    recovery_sources['partial_names'] += 1
                ...
                self.stdout.write(f'Contact {contact.id} has no corresponding Person record')
            except Exception as e:
                self.stdout.write(self.style.ERROR(f'Error processing Contact {contact.id}: {e}'))
        ...
        self.stdout.write(f'Could recover from preferred_name: {recovery_sources["preferred_name"]}')
        self.stdout.write(f'Could recover from title: {recovery_sources["title"]}')
        self.stdout.write(f'Could recover from email: {recovery_sources["email_prefix"]}')
        self.stdout.write(f'Have spouse names (reference): {recovery_sources["spouse_names"]}')
        self.stdout.write(f'Have partial names: {recovery_sources["partial_names"]}')
        
        # Calculate potential recovery rate
        recoverable = recovery_sources['preferred_name'] + recovery_sources['email_prefix'] + recovery_sources['partial_names']
        if total_empty > 0:
        ...
        people_with_preferred = Person.objects.exclude(
            preferred_name__isnull=True
        ).exclude(
            preferred_name=''
        ).filter(
        ...
        self.stdout.write(f'Found {preferred_count} people with preferred_name but missing contact names:')
        
        for person in people_with_preferred[:10]:
            self.stdout.write(f'  Contact {person.contact.id}: preferred_name="{person.preferred_name}"')
