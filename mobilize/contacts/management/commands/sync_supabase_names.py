...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('=== Syncing Names from Supabase People Table ==='))
        ...
        try:
            ...
            if people_with_names:
                self.stdout.write(f'Found {len(people_with_names)} people with names in Supabase people table:')
                
                for contact_id, first_name, last_name in people_with_names:
                    self.stdout.write(f'  Person {contact_id}: "{first_name or ""}" "{last_name or ""}"')
                ...
                for contact_id, p_first, p_last in people_with_names:
                    ...
                    if contact_data:
                        ...
                        if (p_first != c_first) or (p_last != c_last):
                            updates_needed.append((contact_id, p_first, p_last, c_first, c_last, email))
                        ...
                        self.stdout.write(f'  Contact {contact_id}: No corresponding contact record found')
                ...
                if updates_needed:
                    ...
                    if options['dry_run']:
                        ...
                        for contact_id, p_first, p_last, c_first, c_last, email in updates_needed:
                            self.stdout.write(f'  Contact {contact_id}: Update to "{p_first or ""}" "{p_last or ""}"')
                    else:
                        confirm = input(f'Update {len(updates_needed)} contact records? (y/N): ')
                        if confirm.lower() == 'y':
            ...
            if 'column "first_name" does not exist' in str(e) or 'column "last_name" does not exist' in str(e):
                self.stdout.write(self.style.ERROR('The people table does not have first_name/last_name columns.'))
                ...
                self.stdout.write(f'Found {len(empty_name_people)} people with empty names in contacts:')
                
                for contact_id, first_name, last_name, email in empty_name_people:
                    self.stdout.write(f'  Contact {contact_id}: "{first_name or ""}" "{last_name or ""}" ({email})')
