...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('=== Checking Actual Person List API Results ==='))
        ...
        self.stdout.write(f'Testing with user: {user.username} (ID: {user.id}, Role: {user.role})')
        ...
        for person in people_queryset[:20]:  # Check first 20
            ...
            if api_result == "No name listed" or person_str.startswith("Person "):
                self.stdout.write(f'    - ⚠️  PROBLEM: Shows as "{api_result}" or "{person_str}"')
        ...
        for person_id in test_ids:
            ...
            if accessible:
                ...
                try:
                    ...
                    self.stdout.write(f'  Person {person_id}: EXISTS but NOT ACCESSIBLE to this user')
                    self.stdout.write(f'    - Assigned to user: {person.contact.user_id}')
                    self.stdout.write(f'    - Office: {person.contact.office_id}')
                    self.stdout.write(f'    - Names: "{person.contact.first_name}" "{person.contact.last_name}"')
        ...
        if user.role == 'super_admin':
            ...
            for view_mode in ['default', 'my_only']:
                ...
                for person_id in test_ids:
                    ...
                    self.stdout.write(f'    - Person {person_id}: {"✓" if visible else "✗"}')
