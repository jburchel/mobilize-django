...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS("=== Production Database Verification ===")
        )
        ...
        self.stdout.write(
            f'SUPABASE_DATABASE_URL: {"SET" if supabase_database_url else "NOT SET"}'
        )
        ...
        for key, value in db_config.items():
            if key == "PASSWORD":
                self.stdout.write(
                    f'  {key}: {"*" * len(str(value)) if value else "NOT SET"}'
                )
        ...
        try:
            ...
            is_supabase = (
                "supabase" in str(server_addr).lower() or "supabase" in db_name.lower()
            )
            self.stdout.write(
                f'  Appears to be Supabase: {"YES" if is_supabase else "NO"}'
            )
            ...
            if olivia_result:
                ...
                self.stdout.write(
                    f"  Found Olivia Tanchak: Contact {contact_id} ({email})"
                )
            else:
        ...
        if supabase_database_url:
            ...
            try:
                ...
                if "db_name" in locals() and "server_addr" in locals():
                    ...
                    self.stdout.write(
                        f'  Same as Django connection: {"YES" if same_db else "NO"}'
                    )

                    if not same_db:
                        self.stdout.write(
                            self.style.WARNING(
                                "  ⚠️  Django and Supabase connections differ!"
                            )
                        )
                ...
                self.stdout.write(
                    self.style.ERROR(f"✗ Direct Supabase connection failed: {e}")
                )
        ...
        if debug_mode:
            self.stdout.write(
                self.style.WARNING("  ⚠️  DEBUG is True - this may not be production")
            )
        ...
        if render_service_name or render_service_id:
            ...
            if database_url:
                ...
                if "supabase.co" in database_url:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"⚠️  DATABASE_URL does not appear to be Supabase"
                        )
                    )
            else:
        ...
        if database_url and "supabase.co" in database_url:
            ...
            self.stdout.write(
                f"   - Ensure DATABASE_URL points to your Supabase database"
            )
