...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        contacts_without_person = Contact.objects.filter(
            type='person'
        ).exclude(
        ...
        if total_count == 0:
            self.stdout.write(self.style.SUCCESS('All person contacts have Person records!'))
            return
        ...
        if dry_run:
            ...
            for contact in contacts_without_person[:20]:  # Show first 20
                self.stdout.write(f'  Would create Person for: {contact.first_name} {contact.last_name} ({contact.email})')
            if total_count > 20:
        ...
        with transaction.atomic():
            for contact in contacts_without_person:
                try:
                    ...
                    created_count += 1
                    self.stdout.write(f'Created Person for: {contact.first_name} {contact.last_name} ({contact.email})')
