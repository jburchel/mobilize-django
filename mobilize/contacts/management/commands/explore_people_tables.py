...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('=== Exploring People Table Structures ==='))
        ...
        try:
            ...
            if results:
                ...
                for row in results:
                    self.stdout.write(f'  ID {row[0]}: "{row[1] or ""}" "{row[2] or ""}" (contact_id: {row[3]})')
            else:
        ...
        try:
            ...
            if results:
                self.stdout.write('SUCCESS! Found data:')
                for row in results:
                    self.stdout.write(f'  Contact ID {row[0]}: preferred="{row[1] or ""}" title="{row[2] or ""}"')
            else:
                self.stdout.write('No data found with this schema')
        ...
        try:
            ...
            if columns:
                ...
                for col_name, data_type, nullable in columns:
                    self.stdout.write(f'  - {col_name} ({data_type}, nullable: {nullable})')
            else:
        ...
        try:
            ...
            if empty_contacts:
                self.stdout.write(f'Found {len(empty_contacts)} contacts with empty names:')
                ...
                for contact_id, first_name, last_name, email in empty_contacts[:10]:
                    ...
                    try:
                        cursor.execute("SELECT preferred_name FROM people WHERE contact_id = %s", [contact_id])
                        pref_result = cursor.fetchone()
                        if pref_result and pref_result[0]:
                            self.stdout.write(f'    Has preferred_name: "{pref_result[0]}"')
                    except:
                        pass
                    
                    # Approach 2: Look by id (assuming contact_id maps to id)
                    try:
                        cursor.execute("SELECT first_name, last_name FROM people WHERE id = %s", [contact_id])
                        name_result = cursor.fetchone()
                        if name_result and (name_result[0] or name_result[1]):
                            self.stdout.write(f'    Has names in people table: "{name_result[0] or ""}" "{name_result[1] or ""}"')
                    except:
                        pass
