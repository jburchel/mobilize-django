...
class Command(BaseCommand):
    help = 'Fix people with missing names by prompting for names or removing empty records'
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('=== Fixing People with Missing Names ==='))
        ...
        for contact in empty_names:
            try:
                ...
                self.stdout.write(f'Current first_name: "{contact.first_name or "NULL"}"')
                ...
                if options['delete_empty']:
                    ...
                    if not contact.email and not contact.phone and not contact.street_address:
                        should_delete = True
                ...
                if not options['dry_run']:
                    with transaction.atomic():
                        if should_delete:
                            ...
                            deleted_count += 1
                            self.stdout.write(self.style.ERROR(f'DELETED Contact {contact.id}'))
                        else:
                    ...
                    if should_delete:
                        self.stdout.write(self.style.ERROR(f'WOULD DELETE Contact {contact.id}'))
                        deleted_count += 1
                    else:
                        self.stdout.write(self.style.SUCCESS(
