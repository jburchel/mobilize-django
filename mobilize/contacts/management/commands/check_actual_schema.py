...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for col in contacts_columns:
            self.stdout.write(f'{col[0]:25} | {col[1]:15} | Null: {col[2]:5} | Default: {col[3] or "None"}')
        ...
        if sample_people:
            ...
            for person in sample_people:
                self.stdout.write(f'  {person[0]} | {person[1] or "NULL"} | {person[2] or "NULL"}')
        else:
        ...
        if sample_contacts:
            self.stdout.write('Contact samples (id, first_name, last_name, email, type):')
            for contact in sample_contacts:
                self.stdout.write(f'  {contact[0]} | {contact[1] or "NULL"} | {contact[2] or "NULL"} | {contact[3] or "NULL"} | {contact[4]}')
        else:
