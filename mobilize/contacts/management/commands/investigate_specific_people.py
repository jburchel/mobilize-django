...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS("=== Investigating Specific People with Name Issues ===")
        )
        ...
        for person_id in person_ids:
            ...
            try:
                ...
                self.stdout.write(
                    f'  Person preferred_name: "{person.preferred_name or "EMPTY"}"'
                )
                ...
                self.stdout.write(
                    f'  Accessible to super admin: {"Yes" if is_accessible else "No"}'
                )
                ...
                if supabase_data:
                    ...
                    self.stdout.write(
                        f'  Supabase preferred_name: "{sb_preferred or "EMPTY"}"'
                    )
                    self.stdout.write(f'  Supabase title: "{sb_title or "EMPTY"}"')
                    self.stdout.write(
                        f'  Supabase spouse names: "{sb_spouse_first or ""}" "{sb_spouse_last or ""}"'
                    )
                else:
                ...
                if supabase_contact:
                    sb_id, sb_type, sb_first, sb_last, sb_email, sb_user, sb_office = (
                        supabase_contact
                    )
                    self.stdout.write(
                        f'  Supabase contact first_name: "{sb_first or "EMPTY"}"'
                    )
                    self.stdout.write(
                        f'  Supabase contact last_name: "{sb_last or "EMPTY"}"'
                    )
                    self.stdout.write(
                        f'  Supabase contact email: "{sb_email or "EMPTY"}"'
                    )
                    ...
                    if sb_first != contact.first_name or sb_last != contact.last_name:
                        self.stdout.write(
                            "  ⚠️  MISMATCH between Django and Supabase contact data!"
                        )
                else:
                ...
                try:
                    ...
                    self.stdout.write(
                        f"  But Contact {person_id} exists: type={contact.type}"
                    )
                    if contact.type != "person":
                        self.stdout.write(f"  This is a {contact.type}, not a person!")
                except Contact.DoesNotExist:
                    self.stdout.write(
                        f"  Contact {person_id} also does not exist in Django"
                    )

                    # Check Supabase directly
                    cursor.execute(
                        "SELECT id, type FROM contacts WHERE id = %s", [person_id]
                    )
                    supabase_contact = cursor.fetchone()
                    if supabase_contact:
                        sb_id, sb_type = supabase_contact
                        self.stdout.write(
                            f"  But exists in Supabase: id={sb_id}, type={sb_type}"
                        )
        ...
        self.stdout.write(
            f"Found {len(people_with_issues)} people with name issues in first 50:"
        )
        for person in people_with_issues[:10]:  # Show first 10
            self.stdout.write(
                f"  Person {person.contact.id}: email={person.contact.email}"
            )
