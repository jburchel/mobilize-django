...
class Command(BaseCommand):
    help = 'Direct sync from Supabase people table first_name/last_name to contacts table'
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('=== Direct Supabase People Table Sync ==='))
        
        cursor = connection.cursor()
        
        # Direct query attempt - assume the columns exist as you stated
        self.stdout.write('Attempting direct query of people table with first_name/last_name...')
        ...
        try:
            ...
            if people_data:
                ...
                for contact_id, p_first, p_last in people_data:
                    ...
                    if contact_data:
                        ...
                        if (p_first != c_first) or (p_last != c_last):
                            ...
                            if options['fix']:
                                if options['dry_run']:
                                    self.stdout.write(f'    → WOULD UPDATE to: "{p_first}" "{p_last}"')
                                else:
                                    cursor.execute("""
                                        UPDATE contacts 
                                    ...
                                    self.stdout.write(f'    → UPDATED to: "{p_first}" "{p_last}"')
                        else:
                ...
                if options['fix']:
                    ...
                    if all_mismatches:
                        self.stdout.write(f'Found {len(all_mismatches)} total mismatches:')
                        
                        with transaction.atomic():
                            for contact_id, p_first, p_last, c_first, c_last, email in all_mismatches:
                                self.stdout.write(f'\nContact {contact_id}:')
                                self.stdout.write(f'  People: "{p_first or ""}" "{p_last or ""}"')
                                self.stdout.write(f'  Contact: "{c_first or ""}" "{c_last or ""}"')
                                self.stdout.write(f'  Email: {email}')
                ...
                self.stdout.write('No data found for those specific contact IDs in people table')
