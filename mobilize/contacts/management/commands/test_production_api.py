    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for contact_id, first_name, last_name, email, user_id in person_contacts:
                ...
                self.stdout.write(
                    f'    Raw names: "{first_name or ""}" "{last_name or ""}"'
                )
                ...
                if result:
                    ...
                    if contact_type == "person":
                        ...
                        if person_result:
                            ...
                            self.stdout.write(
                                f'    People record: ID {p_id}, names "{p_first or ""}" "{p_last or ""}"'
                            )
                        else:
            ...
            if no_name_contacts:
                self.stdout.write(
                    f"Found {len(no_name_contacts)} contacts with no names:"
                )
                for (
                    contact_id,
                    first_name,
                    ...
                    self.stdout.write(
                        f"  Contact {contact_id}: email={email}, user_id={user_id}"
                    )
            else:
                self.stdout.write(
                    '✓ No contacts found that would show "No name listed"'
                )
