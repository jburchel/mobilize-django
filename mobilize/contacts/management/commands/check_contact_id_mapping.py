    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for name_to_find in test_people:
                ...
                last_name = (
                    name_to_find.split()[1] if len(name_to_find.split()) > 1 else ""
                )
                ...
                if people_results:
                    for p_id, p_first, p_last, contact_id in people_results:
                        self.stdout.write(
                            f"  People table: ID {p_id}, contact_id {contact_id}"
                        )
                        ...
                        if contact_result:
                            ...
                            self.stdout.write(
                                f'    → Contact {c_id}: "{c_first or ""}" "{c_last or ""}" ({c_type}) {email}'
                            )
                        else:
                            self.stdout.write(
                                f"    → Contact {contact_id}: NOT FOUND in contacts table"
                            )
                            ...
                            if contacts_matches:
                                self.stdout.write(
                                    f"    → Found in contacts with different IDs:"
                                )
                                for (
                                    c_id,
                                    c_first,
            ...
            self.stdout.write(
                f"  People table: {total_people} total, {people_with_names} with names"
            )
            self.stdout.write(
                f"  Contacts table: {person_contacts} persons, {empty_name_contacts} with empty names"
            )
            ...
            self.stdout.write(
                f"  Broken links (people with no matching contact): {broken_links}"
            )
            ...
            if broken_links > 0:
                ...
                for p_id, p_first, p_last, contact_id in broken_examples:
                    self.stdout.write(
                        f'  People {p_id}: "{p_first or ""}" "{p_last or ""}" → Contact {contact_id} (missing)'
                    )
