...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS("=== Debugging Django ORM Name Queries ===")
        )
            ...
            try:
                ...
                try:
                    ...
                    if first_name and last_name:
                        ...
                        self.stdout.write(
                            f'    - Result: "{computed_name}" (both names)'
                        )
                    elif first_name:
                        computed_name = first_name
                        self.stdout.write(
                            f'    - Result: "{computed_name}" (first only)'
                        )
                    elif last_name:
                        computed_name = last_name
                        self.stdout.write(
                            f'    - Result: "{computed_name}" (last only)'
                        )
                    else:
        ...
        self.stdout.write(
            f"Query returned {empty_names.count()} contacts with empty names"
        )

        for contact in empty_names:
            self.stdout.write(
                f'  Contact {contact.id}: first="{contact.first_name}", last="{contact.last_name}"'
            )
        ...
        self.stdout.write(
            f"Query returned {both_empty.count()} contacts with both names empty"
        )
        ...
        for contact_id in test_ids:
            try:
                ...
                self.stdout.write(
                    f'  - Processed: "{first_name}" + "{last_name}" = "{full_name}"'
                )
