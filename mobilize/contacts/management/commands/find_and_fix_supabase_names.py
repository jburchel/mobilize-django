...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS("=== Finding and Fixing Supabase Name Mismatches ===")
        )
        ...
        if not people_name_columns:
            self.stdout.write(
                "People table does not have first_name/last_name columns."
            )
            ...
            if preferred_name_records:
                self.stdout.write(
                    f"Found {len(preferred_name_records)} people with preferred_name but no contact names:"
                )
                for record in preferred_name_records:
                    contact_id, preferred_name, c_first, c_last, email = record
                    self.stdout.write(
                        f'  Contact {contact_id}: preferred_name="{preferred_name}", email="{email}"'
                    )
                    ...
                    if options["fix"] and not options["dry_run"]:
                        ...
                        self.stdout.write(
                            f'    → Fixed: Set first_name to "{preferred_name}"'
                        )
            else:
            ...
            self.stdout.write(
                f"People table has these name columns: {people_name_columns}"
            )
            ...
            if mismatch_records:
                ...
                for record in mismatch_records:
                    ...
                    if options["fix"]:
                        if options["dry_run"]:
                            self.stdout.write(
                                f'  → WOULD UPDATE contact table to: "{p_first or ""}" "{p_last or ""}"'
                            )
                        else:
                            cursor.execute(
                                """
                            ...
                            self.stdout.write(
                                f'  → UPDATED contact table to: "{p_first or ""}" "{p_last or ""}"'
                            )
            else:
                self.stdout.write(
                    "No name mismatches found between people and contacts tables."
                )
        ...
        self.stdout.write(
            f"Found {people_with_empty_names.count()} people with empty names in Django:"
        )

        for person in people_with_empty_names:
            self.stdout.write(
                f'  Person {person.contact.id}: "{str(person)}" (email: {person.contact.email})'
            )
            ...
            if person_data:
                ...
                if preferred_name:
                    ...
                    if options["fix"] and not options["dry_run"]:
                        ...
                        self.stdout.write(
                            f'    → WOULD FIX: Set first_name to "{preferred_name}"'
                        )
        ...
        if options["dry_run"]:
            ...
            self.stdout.write(
                "Investigation completed. Use --fix --dry-run to see proposed changes."
            )
