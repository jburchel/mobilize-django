    ...
    def handle(self, *args, **options):
        ...
        for contact_id in test_contacts:
            try:
                ...
                self.stdout.write(f'  Assigned to user: {contact.user_id} ({contact.user.username if contact.user else "No user"})')
                self.stdout.write(f'  Office: {contact.office_id} ({contact.office.name if contact.office else "No office"})')
        ...
        for user in users:
            self.stdout.write(f'  User {user.id}: {user.username} ({user.first_name} {user.last_name}) - Role: {user.role}')
        ...
        if options['user_id']:
            ...
            try:
                ...
                for contact_id in test_contacts:
                    ...
                    self.stdout.write(f'  Contact {contact_id}: {"✓ VISIBLE" if is_visible else "✗ HIDDEN"}')
                
                # Show first few visible people
                self.stdout.write('\\nFirst 10 visible people:')
                for person in people_queryset[:10]:
                    self.stdout.write(f'  {person.contact.id}: {person.name} (user: {person.contact.user_id})')
            ...
            for user in users:
                ...
                self.stdout.write(f'  User {user.id} ({user.username}): sees {visible_count} people')
                self.stdout.write(f'    - Can see Olivia (685): {"Yes" if can_see_685 else "No"}')
                self.stdout.write(f'    - Can see Nate (699): {"Yes" if can_see_699 else "No"}')
