...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('=== Syncing Production Supabase Names ==='))
        ...
        try:
            ...
            if 'first_name' in column_names:
                self.stdout.write('✓ Production people table has first_name/last_name columns!')
                ...
                if people_with_names:
                    self.stdout.write(f'Found {len(people_with_names)} people with names in production:')
                    ...
                    for row in people_with_names:
                        ...
                        self.stdout.write(f'\nPeople ID {p_id}:')
                        self.stdout.write(f'  Names: "{p_first_display}" "{p_last_display}"')
                        self.stdout.write(f'  Contact ID: {contact_id}')
                        
                        if contact_id and email:
                            self.stdout.write(f'  Contact names: "{c_first_display}" "{c_last_display}"')
                            ...
                            if (p_first != c_first) or (p_last != c_last):
                                ...
                                if not c_first and not c_last:
                                    self.stdout.write(f'  → This shows as "No name listed" in the app!')
                                
                                mismatches.append((contact_id, p_first, p_last, c_first, c_last, email))
                            else:
                                self.stdout.write(f'  ✓ Names match')
                        else:
                            self.stdout.write(f'  ✗ No linked contact or contact not found')
                    ...
                    if mismatches:
                        ...
                        if options['fix']:
                            if options['dry_run']:
                                ...
                                for contact_id, p_first, p_last, c_first, c_last, email in mismatches:
                                    p_first_safe = p_first or ""
                                    p_last_safe = p_last or ""
                                    self.stdout.write(f'  Contact {contact_id}: Update to "{p_first_safe}" "{p_last_safe}"')
                            else:
                                confirm = input(f'Apply {len(mismatches)} name fixes to production? (y/N): ')
                                if confirm.lower() == 'y':
                                    ...
                                    for contact_id, p_first, p_last, c_first, c_last, email in mismatches:
                                        ...
                                        p_first_safe = p_first or ""
                                        p_last_safe = p_last or ""
                                        self.stdout.write(f'✓ Updated Contact {contact_id}: "{p_first_safe}" "{p_last_safe}"')
                                    
                                    conn.commit()
                                    self.stdout.write(self.style.SUCCESS(f'✓ Successfully updated {len(mismatches)} contacts!'))
                                else:
                                    self.stdout.write('Fixes cancelled.')
                        else:
                            self.stdout.write('Use --fix to apply changes, or --fix --dry-run to preview.')
                    else:
                        self.stdout.write('\n✓ No mismatches found - all names are synced!')
                        
                else:
                    self.stdout.write('No people with names found in production people table.')
                    
            else:
                self.stdout.write('✗ Production people table does not have first_name/last_name columns')
            ...
            self.stdout.write(self.style.ERROR(f'Error connecting to production database: {e}'))
