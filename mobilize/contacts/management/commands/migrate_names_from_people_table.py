...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS(
                "=== Migrating Names from People Table to Contacts Table ==="
            )
        )
        ...
        if "first_name" not in people_columns or "last_name" not in people_columns:
            self.stdout.write(
                self.style.ERROR(
                    "People table does not have first_name and last_name columns!"
                )
            )
            return
        ...
        self.stdout.write(
            f"Found {total_mismatched} people with names in people table but missing in contacts table:"
        )
        ...
        for record in mismatched_records:
            ...
            self.stdout.write(
                f'  → Will update contact table to: "{p_first or ""}" "{p_last or ""}"'
            )
        ...
        if options["dry_run"]:
            ...
            self.stdout.write(
                f"Run without --dry-run to apply {total_mismatched} updates"
            )
            return
        ...
        with transaction.atomic():
            for record in mismatched_records:
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"✗ Failed to update Contact {contact_id}: {e}"
                        )
                    )
        ...
        if remaining_issues == 0:
            self.stdout.write(
                self.style.SUCCESS("✓ All name data successfully migrated!")
            )
        else:
            self.stdout.write(
                self.style.WARNING(
                    f"⚠ {remaining_issues} records still have name issues"
                )
            )
