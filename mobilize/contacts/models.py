...
ALL_PIPELINE_STAGES = list(
    set(MAIN_PEOPLE_PIPELINE_STAGES + MAIN_CHURCH_PIPELINE_STAGES)
)
...
class Contact(models.Model):
    ...
    notes = models.TextField(
        blank=True, null=True, help_text="General notes about the contact."
    )
    initial_notes = models.TextField(
        blank=True, null=True
    )  # Consider merging into 'notes'
    ...
    conflict_data = models.JSONField(
        blank=True, null=True
    )  # Using JSONField for jsonb type
    ...
    last_contact_date = models.DateTimeField(
        blank=True, null=True, help_text="Date of last interaction."
    )
    next_contact_date = models.DateTimeField(
        blank=True, null=True, help_text="Scheduled date for next interaction."
    )
    tags = models.JSONField(
        blank=True, null=True, help_text="JSON field for storing tags."
    )  # Overwrites existing tags field
    custom_fields = models.JSONField(
        blank=True, null=True, help_text="JSON field for custom data."
    )
    ...
    def full_address(self):
        ...
        if (
            self.country
            and self.country.lower() != "usa"
            and self.country.lower() != "united states"
        ):
    ...
    def set_pipeline_stage(self, stage_name):
        ...
        pipeline_contact, created = PipelineContact.objects.get_or_create(
            ...
            defaults={"contact_type": self.type, "current_stage": stage},
        )
...
class Person(models.Model):
    ...
    languages = models.JSONField(
        blank=True, null=True
    )  # Changed to JSONField as per mobilize-prompt-django.md

    # Professional details
    profession = models.CharField(
        max_length=100, blank=True, null=True
    )  # Renamed from occupation
    organization = models.CharField(
        max_length=255, blank=True, null=True
    )  # Renamed from employer
    ...
    info_given = models.TextField(
        blank=True, null=True, help_text="Information about mission/vision shared"
    )
    desired_service = models.TextField(
        blank=True, null=True, help_text="Desired service location or type"
    )
    ...
    google_contact_id = models.CharField(
        max_length=255, blank=True, null=True
    )  # From mobilize-prompt-django.md
    ...
    class Meta:
        ...
        ordering = [
            "contact__last_name",
            "contact__first_name",
        ]  # Order by related Contact fields
