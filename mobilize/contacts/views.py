...
def person_list(request):
    ...
    if use_lazy_loading:
        ...
        people = access_manager.get_people_queryset().select_related(
        ...
        if query:
            people = people.filter(
                ...
                | Q(contact__notes__icontains=query)
                | Q(contact__initial_notes__icontains=query)
            )
        ...
        if pipeline_stage:
            ...
            if main_pipeline:
                ...
                pipeline_contacts = PipelineContact.objects.filter(
                    ...
                    current_stage__name__iexact=pipeline_stage.replace(
                        "_", " "
                    ).title(),
        ...
        if sort_by in sort_options_mapping:
            ...
            people = people.order_by(
                "contact__last_name", "contact__first_name", "pk"
            )  # Default sort
...
def person_detail(request, pk):
    ...
    try:
        ...
        person = get_object_or_404(
            Person.objects.select_related("contact", "contact__office"), pk=pk
        )
        ...
        recent_communications = Communication.objects.filter(
            Q(person__contact_id=person.contact.id)
            | Q(church__contact_id=person.contact.id)
        ).order_by("-date_sent", "-created_at")[:5]

        # Get related tasks for this person
        related_tasks = person.person_tasks.all().order_by("-due_date", "-created_at")[
            :10
        ]
        ...
        if main_pipeline:
            ...
            try:
                pipeline_contact = PipelineContact.objects.get(
                    contact=person.contact, pipeline=main_pipeline
                )
...
def person_delete(request, pk):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(
                request, f"Error deleting {name}. Please try again or contact support."
            )
            return render(
                request,
                "contacts/person_confirm_delete.html",
                {
                    "person": person,
                },
            )
...
def person_list_api(request):
    ...
    # Debug logging
    logger.info(
        f"🔍 DEBUG: User {request.user.email} (Role: {request.user.role}) accessing person_list_api with view_mode: {access_manager.view_mode}"
    )
    ...
    try:
        ...
        logger.info(
            f"🔍 DEBUG: Initial people count after access filtering: {initial_count}"
        )
    ...
    try:
        ...
        if sort_by in sort_options_mapping:
            people = people.order_by(sort_options_mapping[sort_by], "pk")
            logger.info(
                f"🔍 DEBUG: Applied sorting: {sort_by} -> {sort_options_mapping[sort_by]}"
            )
        else:
            people = people.order_by(
                "contact__last_name", "contact__first_name", "pk"
        ...
        try:
            ...
            logger.info(
                f"🔍 DEBUG: Total people count: {total_count}, per_page: {per_page}"
            )
        except Exception as e:
        ...
        try:
            ...
            logger.info(
                f"🔍 DEBUG: Page {page}: requested {per_page} items, got {len(people_to_process)} people (indices {start_idx}:{end_idx} of {total_count} total)"
            )
    ...
    for i, person in enumerate(people_to_process):
        try:
            logger.info(
                f"🔍 DEBUG: Processing person {i+1}: ID={person.pk}, Contact ID={person.contact.id}"
            )
            ...
            logger.info(
                f"🔍 DEBUG: Raw names - First: '{first_name}', Last: '{last_name}'"
            )
            ...
            logger.info(
                f"🔍 DEBUG: Person name: '{full_name}', Email: '{person.contact.email}'"
            )
    ...
    if len(results) > per_page:
        logger.warning(
            f"⚠️ WARNING: Returning {len(results)} results but per_page is {per_page}"
        )
                    ...
                    try:
                        ...
                        if email:
                            ...
                            try:
                                ...
                                contact.street_address = row.get(
                                    "street_address", row.get("address_line1", "")
                                ).strip()
                                contact.city = row.get("city", "").strip()
                                contact.state = row.get("state", "").strip()
                                contact.zip_code = row.get(
                                    "zip_code", row.get("postal_code", "")
                                ).strip()
                                contact.country = row.get(
                                    "country", "United States"
                                ).strip()
                                contact.pipeline_stage = row.get(
                                    "pipeline_stage", "new"
                                ).strip()
...
def export_contacts(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="contacts_export_{timestamp}.csv"'
    )
    ...
    for person in people:
        writer.writerow(
            [
                ...
                (
                    person.contact.created_at.strftime("%Y-%m-%d %H:%M:%S")
                    if person.contact.created_at
                    else ""
                ),
                (
                    person.contact.updated_at.strftime("%Y-%m-%d %H:%M:%S")
                    if person.contact.updated_at
                    else ""
                ),
            ]
        )
...
def google_sync(request):
    ...
    try:
        ...
        sync_settings = UserContactSyncSettings.objects.create(
            user=request.user, sync_preference="crm_only", auto_sync_enabled=False
        )
    ...
    if request.method == "POST":
        ...
        if action == "sync":
            if not contacts_service.is_authenticated():
                messages.error(
                    request,
                    "Google Contacts not authenticated. Please connect Gmail first.",
                )
            ...
            if result["success"]:
                ...
                if "created_count" in result and "updated_count" in result:
                    messages.info(
                        request,
                        f"Created {result['created_count']} new contacts, updated {result['updated_count']} existing contacts",
                    )
                ...
                messages.error(
                    request,
                    f"Contact sync failed: {result.get('error', 'Unknown error')}",
                )
...
def selective_google_import(request):
    ...
    if not contacts_service.is_authenticated():
        messages.error(
            request, "Google Contacts not authenticated. Please connect Gmail first."
        )
        return redirect("communications:gmail_auth")
...
def bulk_update_priority(request):
    ...
    try:
        ...
        messages.success(
            request,
            f"Successfully updated {count} contact(s) to {priority_display} priority",
        )
...
def bulk_assign_office(request):
    ...
    try:
        ...
        messages.success(
            request, f"Successfully assigned {count} contact(s) to {office.name}"
        )
...
def bulk_assign_user(request):
    ...
    try:
        ...
        messages.success(
            request, f"Successfully assigned {count} contact(s) to {user_display_name}"
        )
