...
def google_auth_callback(request):
    ...
    try:
        ...
        if token_response.status_code != 200:
            ...
            messages.error(
                request, "Failed to authenticate with Google. Please try again."
            )
            return redirect("authentication:login")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            UserContactSyncSettings.objects.update_or_create(
                user=request.user, defaults={"sync_preference": sync_preference}
            )
...
class UserListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = (
            User.objects.all()
            .select_related()
            .prefetch_related("useroffice_set__office")
            .annotate(office_count=Count("useroffice", distinct=True))
        )
