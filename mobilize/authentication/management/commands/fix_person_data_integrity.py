...
class Command(BaseCommand):
    help = 'Fix Person table data integrity issues - NULL PKs and broken FK relationships'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("🔧 Diagnosing Person table data integrity issues"))
    ...
    def analyze_person_table(self):
        ...
        with connection.cursor() as cursor:
            ...
            if pk_column:
                ...
                self.stdout.write(f"   Person records with NULL {pk_column}: {null_pk_count}")
            ...
            if has_contact_id_column:
                ...
                self.stdout.write(f"   Person records with NULL contact_id: {null_contact_id_count}")
            ...
            if pk_column and has_contact_id_column:
                ...
                if problematic_records:
                    ...
                    for record in problematic_records:
                        self.stdout.write(f"     {pk_column}: {record[0]}, Contact ID: {record[1]}, Title: {record[2]}")
    ...
    def fix_null_primary_keys(self, dry_run):
        ...
        with connection.cursor() as cursor:
            ...
            if null_pk_count > 0:
                self.stdout.write(f"   Found {null_pk_count} Person records with NULL {pk_column}")
                
                if not dry_run:
                    # Get the next available ID from sequence
                    cursor.execute(f"SELECT COALESCE(MAX({pk_column}), 0) + 1 FROM people WHERE {pk_column} IS NOT NULL")
                    ...
                    cursor.execute(f"SELECT setval('{seq_name}', (SELECT MAX({pk_column}) FROM people))")
                    
                    self.stdout.write(self.style.SUCCESS(f"   ✅ Fixed NULL {pk_column} starting from ID {next_id}"))
                else:
    ...
    def rebuild_person_contact_relationships(self, dry_run):
        ...
        with connection.cursor() as cursor:
            ...
            if invalid_relationships:
                self.stdout.write(f"   Found {len(invalid_relationships)} Person records with invalid contact_id")
                
                for person_id, contact_id, title, profession in invalid_relationships:
                    self.stdout.write(f"     Person {pk_column} {person_id}: contact_id={contact_id}, title={title}")
                ...
                if not dry_run:
                    ...
                    for person_id, contact_id, title, profession in invalid_relationships:
                        ...
                        self.stdout.write(f"     ✅ Created Contact {new_contact_id} for Person {person_id}")
                else:
                    self.stdout.write(f"   Would create {len(invalid_relationships)} new Contact records")
            else:
    ...
    def verify_fixes(self):
        ...
        try:
            ...
            self.stdout.write(f"   Successfully evaluated queryset: {len(test_people)} people")
            
            # Test foreign key access
            for i, person in enumerate(test_people):
                try:
                    contact = person.contact
                    self.stdout.write(f"   Person {person.pk}: contact={contact.id}, name={contact.first_name}")
                except Exception as e:
                    self.stdout.write(self.style.ERROR(f"   ❌ Person {person.pk}: FK access failed - {e}"))
