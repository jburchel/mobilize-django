...
class Command(BaseCommand):
    help = (
        "Comprehensive schema synchronization between local Django models and Supabase"
    )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS("🚀 Starting Comprehensive Schema Synchronization")
        )
    ...
    def analyze_current_state(self, verbose):
        ...
        with connection.cursor() as cursor:
            ...
            if verbose:
                ...
                self.stdout.write(
                    f"ℹ️  Extra {len(extra_tables)} tables (may be legacy)"
                )
                ...
                if table_name in critical_tables:
                    ...
                    if not dry_run:
                        try:
                            ...
                            self.stdout.write(
                                self.style.ERROR(
                                    f"❌ Failed to create {table_name}: {e}"
                                )
                            )
                    else:
                    ...
                    self.stdout.write(
                        f"⚠️  Skipping {table_name} (no schema definition)"
                    )
                ...
                if table_name in self.existing_tables:
                    ...
                    if missing_columns:
                        ...
                        for column_name in missing_columns:
                            ...
                            if verbose:
                                self.stdout.write(
                                    f"  + Adding {column_name} {column_def}"
                                )
                            ...
                            if not dry_run:
                                try:
                                    ...
                                    self.stdout.write(
                                        self.style.ERROR(
                                            f"❌ Failed to add {table_name}.{column_name}: {e}"
                                        )
                                    )
                            else:
                                self.stdout.write(
                                    f"WOULD ADD: {table_name}.{column_name}"
                                )
                    elif verbose:
                        self.stdout.write(
    ...
    def fix_people_table_schema(self, dry_run, verbose):
        ...
        with connection.cursor() as cursor:
            ...
            if has_id_column and not has_contact_id_column:
                if verbose:
                    self.stdout.write(
                        "🔧 CRITICAL: People table has 'id' but needs 'contact_id'"
                    )
                ...
                if not dry_run:
                    try:
                        ...
                        if aligned_count == total_count and total_count > 0:
                            if verbose:
                                self.stdout.write(
                                    f"✅ Data alignment verified ({aligned_count}/{total_count})"
                                )

                            # Rename the column
                            cursor.execute(
                                "ALTER TABLE people RENAME COLUMN id TO contact_id;"
                            )
                            if verbose:
                                self.stdout.write(
                                    "✅ Renamed 'id' to 'contact_id' in people table"
                                )
                            ...
                            for col in legacy_columns:
                                if col in column_names:
                                    cursor.execute(
                                        f"ALTER TABLE people DROP COLUMN IF EXISTS {col};"
                                    )
                                    if verbose:
                                        self.stdout.write(
                                            f"✅ Dropped legacy column: {col}"
                                        )
                        else:
                            self.stdout.write(
                                self.style.ERROR(
                                    f"❌ Data alignment mismatch: {aligned_count}/{total_count}"
                                )
                            )
                        ...
                        self.stdout.write(
                            self.style.ERROR(
                                f"❌ Failed to fix people table schema: {e}"
                            )
                        )
                else:
                    self.stdout.write(
                        "WOULD FIX: Rename people.id to people.contact_id"
                    )
                ...
                if verbose:
                    ...
                    try:
                        ...
                        self.stdout.write(
                            f"✅ Django Person.objects.count() = {test_count}"
                        )

                        # Test queryset evaluation
                        test_people = list(Person.objects.all()[:3])
                        self.stdout.write(
                            f"✅ Django queryset evaluation works - got {len(test_people)} people"
                        )
                    except Exception as e:
                        self.stdout.write(
                            self.style.ERROR(f"❌ Django ORM test failed: {e}")
                        )
            else:
                if verbose:
                ...
                if old_exists and not new_exists:
                    ...
                    if not dry_run:
                        try:
                            ...
                            self.stdout.write(
                                self.style.ERROR(f"❌ Failed to rename {old_name}: {e}")
                            )
                    else:
                    ...
                    if verbose:
                        self.stdout.write(
                            f"⚠️  Both {old_name} and {new_name} exist - data migration needed"
                        )
                    ...
                    if not dry_run:
                        ...
                        if old_count > 0 and new_count == 0:
                            self.stdout.write(
                                f"📊 Migrating {old_count} records from {old_name} to {new_name}"
                            )
                            ...
                            self.stdout.write(
                                f"⚠️  Manual review needed for {old_name} ({old_count} records) vs {new_name} ({new_count} records)"
                            )
            ...
            for table_name, table_indexes in indexes.items():
                if table_name in self.existing_tables:
                    for index_sql in table_indexes:
                        if verbose:
                            index_name = (
                                index_sql.split("IF NOT EXISTS")[1]
                                .split("ON")[0]
                                .strip()
                            )
    ...
    def verify_schema_integrity(self, verbose):
        ...
        with connection.cursor() as cursor:
            ...
            critical_checks = [
                ...
                (
                    "people table has contact_id",
                    "SELECT contact_id FROM people LIMIT 1;",
                ),
                (
                    "churches table has contact_id",
                    "SELECT contact_id FROM churches LIMIT 1;",
                ),
                (
                    "pipeline_pipelinecontact exists",
                    "SELECT COUNT(*) FROM pipeline_pipelinecontact LIMIT 1;",
                ),
                (
                    "django_session table exists",
                    "SELECT COUNT(*) FROM django_session LIMIT 1;",
                ),
            ]
    ...
    def fix_person_data_integrity(self, dry_run, verbose):
        ...
        with connection.cursor() as cursor:
            ...
            if null_pk_count > 0:
                self.stdout.write(
                    f"   🚨 CRITICAL: Found {null_pk_count} Person records with NULL {pk_column}"
                )
                ...
                if not dry_run:
                    try:
                        ...
                        cursor.execute(
                            f"SELECT COUNT(*) FROM people WHERE {pk_column} IS NULL"
                        )
                        ...
                        if fixed_count > 0:
                            self.stdout.write(
                                f"   ✅ Fixed {fixed_count} NULL {pk_column} values"
                            )
                        if remaining_nulls > 0:
                            self.stdout.write(
                                f"   ⚠️  {remaining_nulls} NULL {pk_column} values remain"
                            )

                    except Exception as e:
                        self.stdout.write(
                            self.style.ERROR(
                                f"   ❌ Failed to fix NULL primary keys: {e}"
                            )
                        )
                else:
                    self.stdout.write(
                        f"   WOULD FIX: {null_pk_count} NULL {pk_column} values"
                    )
            else:
                if verbose:
            ...
            if broken_fk_count > 0:
                self.stdout.write(
                    f"   🚨 Found {broken_fk_count} Person records with invalid contact_id"
                )
                ...
                if not dry_run:
                    try:
                        ...
                        for person_pk, contact_id, title, profession in orphaned_people:
                            ...
                            if verbose:
                                self.stdout.write(
                                    f"   ✅ Created Contact {new_contact_id} for Person {person_pk}"
                                )

                        self.stdout.write(
                            f"   ✅ Fixed {len(orphaned_people)} broken Person-Contact relationships"
                        )

                    except Exception as e:
                        self.stdout.write(
                            self.style.ERROR(
                                f"   ❌ Failed to fix broken relationships: {e}"
                            )
                        )
                else:
                    self.stdout.write(
                        f"   WOULD FIX: {broken_fk_count} broken Person-Contact relationships"
                    )
            else:
                if verbose:
                    self.stdout.write(
                        "   ✅ All Person-Contact relationships are valid"
                    )
            ...
            try:
                ...
                self.stdout.write(
                    f"   📊 Django Person.objects.count() = {django_count}"
                )
                ...
                for person in test_people:
                    try:
                        ...
                        if verbose:
                            self.stdout.write(
                                f"   ✅ Person {person.pk}: contact={contact.id}"
                            )
                    except Exception:
                        pass
                ...
                if success_count == len(test_people) and len(test_people) > 0:
                    self.stdout.write(
                        f"   ✅ All {success_count} Person-Contact FK relationships work correctly"
                    )
                elif success_count > 0:
                    self.stdout.write(
                        f"   ⚠️  {success_count}/{len(test_people)} Person-Contact FK relationships work"
                    )
                else:
                ...
                self.stdout.write(
                    self.style.ERROR(f"   ❌ Django ORM verification failed: {e}")
                )
    ...
    def fix_church_data_integrity(self, dry_run, verbose):
        ...
        with connection.cursor() as cursor:
            ...
            if null_pk_count > 0:
                self.stdout.write(
                    f"   🚨 CRITICAL: Found {null_pk_count} Church records with NULL {pk_column}"
                )
                ...
                if not dry_run:
                    try:
                        ...
                        # Check how many were fixed
                        cursor.execute(
                            f"SELECT COUNT(*) FROM churches WHERE {pk_column} IS NULL"
                        )
                        remaining_nulls = cursor.fetchone()[0]
                        fixed_count = null_pk_count - remaining_nulls
            ...
            if broken_fk_count > 0:
                self.stdout.write(
                    f"   🚨 Found {broken_fk_count} Church records with invalid contact_id"
                )
                ...
                if not dry_run:
                    try:
                        ...
                        for church_pk, contact_id, name, location in orphaned_churches:
                            ...
                            if verbose:
                                self.stdout.write(
                                    f"   ✅ Created Contact {new_contact_id} for Church {church_pk}"
                                )

                        self.stdout.write(
                            f"   ✅ Fixed {len(orphaned_churches)} broken Church-Contact relationships"
                        )
                    ...
                    self.stdout.write(
                        f"   WOULD FIX: {broken_fk_count} broken Church-Contact relationships"
                    )
            else:
                if verbose:
                    self.stdout.write(
                        "   ✅ All Church-Contact relationships are valid"
                    )
            ...
            try:
                ...
                self.stdout.write(
                    f"   📊 Django Church.objects.count() = {django_count}"
                )

                # Test queryset evaluation
                test_churches = list(Church.objects.all()[:3])
                self.stdout.write(
                    f"   ✅ Django queryset evaluation works - got {len(test_churches)} churches"
                )
                ...
                for church in test_churches:
                    try:
                        ...
                        if verbose:
                            self.stdout.write(
                                f"   ✅ Church {church.pk}: contact={contact.id}"
                            )
                    except Exception:
                        pass
                ...
                if success_count == len(test_churches) and len(test_churches) > 0:
                    self.stdout.write(
                        f"   ✅ All {success_count} Church-Contact FK relationships work correctly"
                    )
                elif success_count > 0:
                    self.stdout.write(
                        f"   ⚠️  {success_count}/{len(test_churches)} Church-Contact FK relationships work"
                    )
                else:
    ...
    def fix_pipeline_foreign_keys(self, dry_run, verbose):
        ...
        with connection.cursor() as cursor:
            ...
            if not table_exists:
                if verbose:
                    self.stdout.write(
                        "   ⚠️  pipeline_pipelinecontact table does not exist, skipping FK fixes"
                    )
                return
            ...
            if verbose:
                self.stdout.write(
                    f"   📊 Found {len(existing_fks)} existing foreign keys: {list(existing_fk_columns)}"
                )
            ...
            for column_name, ref_table, ref_column in required_fks:
                if column_name not in existing_fk_columns:
                    if verbose:
                        self.stdout.write(
                            f"   🔧 Adding foreign key: {column_name} -> {ref_table}({ref_column})"
                        )
                    ...
                    if not dry_run:
                        try:
                            ...
                            constraint_name = (
                                f"pipeline_pipelinecontact_{column_name}_fkey"
                            )
                            cursor.execute(
                                f"""
                                ALTER TABLE pipeline_pipelinecontact 
                            ...
                            self.stdout.write(
                                f"   ✅ Added foreign key constraint for {column_name}"
                            )

                        except Exception as e:
                            self.stdout.write(
                                self.style.ERROR(
                                    f"   ❌ Failed to add FK for {column_name}: {e}"
                                )
                            )
                    else:
                        self.stdout.write(
                            f"   WOULD ADD FK: {column_name} -> {ref_table}({ref_column})"
                        )
                elif verbose:
                    self.stdout.write(
                        f"   ✅ Foreign key for {column_name} already exists"
                    )
