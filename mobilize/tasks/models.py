...
from mobilize.contacts.models import (
    Person,
)  # Contact model is not directly used here, Person and Church are.
...
class Task(models.Model):
    ...
    reminder_option = models.CharField(
    ...
    priority = models.CharField(
        max_length=50, choices=PRIORITY_CHOICES, default="medium", blank=True, null=True
    )
    status = models.CharField(
        max_length=50, choices=STATUS_CHOICES, default="pending", blank=True, null=True
    )
    ...
    created_by = models.ForeignKey(
        ...
        null=True,
        blank=True,  # Assuming created_by can be null if user is deleted or task is system-generated
    )
    assigned_to = models.ForeignKey(
        ...
        null=True,
        blank=True,  # A task might not be assigned initially
    )
    ...
    recurrence_end_date = models.DateField(
        blank=True, null=True, help_text="Date when the recurring series should end."
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=["assigned_to"]
            ),  # Index on ForeignKey is often created automatically by DB
    ...
    def calculate_next_occurrence(self):
        ...
        if frequency == "daily":
            ...
            if day_of_month:
                next_date = base_date.replace(day=day_of_month) + relativedelta(
                    months=interval
                )
            else:
    ...
    def generate_pending_occurrences(cls, days_ahead=7):
        ...
        templates = cls.objects.filter(
            is_recurring_template=True, next_occurrence_date__lte=cutoff_date
        ).exclude(recurrence_end_date__lt=timezone.now().date())

        generated_count = 0
        for template in templates:
            while (
                template.next_occurrence_date
                and template.next_occurrence_date <= cutoff_date
                and (
                    not template.recurrence_end_date
                    or template.next_occurrence_date.date()
...
class RecurringTaskTemplate(models.Model):
    ...
    # Task properties
    priority = models.CharField(
        max_length=50, choices=Task.PRIORITY_CHOICES, default="medium"
    )
    category = models.CharField(max_length=255, blank=True, null=True)
    ...
    last_created = models.DateTimeField(
        null=True,
        blank=True,
    ...
    send_notifications = models.BooleanField(
        default=True, help_text="Send assignment notifications for created tasks"
    )
    ...
    def should_create_new_task(self, current_time):
        ...
        if frequency == "daily":
            ...
            months_diff = (current_time.year - self.last_created.year) * 12 + (
                current_time.month - self.last_created.month
            )
    ...
    def calculate_next_due_date(self, base_time):
        ...
        """
        pattern = self.recurrence_pattern
        due_time = pattern.get(
            "due_time", "end_of_day"
        )  # 'start_of_day', 'end_of_day', or specific time
        days_ahead = pattern.get(
            "days_ahead", 0
        )  # How many days ahead to set the due date
