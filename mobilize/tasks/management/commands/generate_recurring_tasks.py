    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            ...
            for template in templates:
                ...
                while (
                    temp_next_date
                    and temp_next_date <= cutoff_date
                    and (
                        not template.recurrence_end_date
                        or temp_next_date.date() <= template.recurrence_end_date
                    )
                ):
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"[DRY RUN] Would generate {count} recurring task occurrences"
                )
            )
        else:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully generated {generated_count} recurring task occurrences"
                )
            )
            ...
            active_templates = (
                Task.objects.filter(
                    is_recurring_template=True, next_occurrence_date__isnull=False
                )
                .exclude(recurrence_end_date__lt=timezone.now().date())
                .count()
            )
