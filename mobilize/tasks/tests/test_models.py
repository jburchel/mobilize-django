...
class TaskModelTest(TestCase):
    ...
    def setUp(self):
        ...
        person_contact = Contact.objects.create(
            type="person", first_name="John", last_name="Doe", email="john@example.com"
        )
        self.person = Person.objects.create(contact=person_contact)
        # Create Contact first, then Church
        church_contact = Contact.objects.create(
            type="church", church_name="Test Church"
        )
        self.church = Church.objects.create(contact=church_contact, name="Test Church")
    ...
    def test_task_str_representation(self):
        ...
        task = Task.objects.create(
            title="Test Task", created_by=self.user, office=self.office
        )
        self.assertEqual(str(task), "Test Task")

        # Test with no title
        task_no_title = Task.objects.create(created_by=self.user, office=self.office)
    ...
    def test_task_office_relationship(self):
        ...
        task = Task.objects.create(
            title="Office Task", created_by=self.user, office=self.office
        )
    ...
    def test_task_get_absolute_url(self):
        ...
        task = Task.objects.create(
            title="URL Task", created_by=self.user, office=self.office
        )
...
class TaskRecurringTest(TestCase):
    ...
    def test_calculate_next_occurrence_daily(self):
        ...
        template = Task.objects.create(
            ...
            is_recurring_template=True,
            next_occurrence_date=timezone.make_aware(
    ...
    def test_calculate_next_occurrence_monthly(self):
        ...
        pattern = {"frequency": "monthly", "interval": 1, "day_of_month": 15}
...
class TaskCascadeDeleteTest(TestCase):
    ...
    def test_office_deletion_sets_null(self):
        ...
        task = Task.objects.create(
            title="Office Task", office=self.office, created_by=self.user
        )
