...
class TaskListViewTest(TestCase):
    ...
    def test_task_list_ordering(self):
        ...
        pending_indices = [
            i for i, status in enumerate(task_statuses) if status == "pending"
        ]
        completed_indices = [
            i for i, status in enumerate(task_statuses) if status == "completed"
        ]
    ...
    def test_task_list_pagination(self):
        ...
        for i in range(20):
            Task.objects.create(
                title=f"Task {i}", created_by=self.user, office=self.office
            )
...
class TaskViewIntegrationTest(TestCase):
    ...
    def test_recurring_task_workflow(self):
        ...
        template = Task.objects.filter(
            title="Daily Standup", is_recurring_template=True
        ).first()
