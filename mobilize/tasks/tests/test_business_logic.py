...
class TaskRecurringBusinessLogicTest(TestCase):
    ...
    def test_daily_recurrence_calculation(self):
        ...
        template = Task.objects.create(
            ...
            next_occurrence_date=timezone.make_aware(
                datetime.combine(date(2024, 1, 1), datetime.min.time())
            ),
    ...
    def test_generate_occurrence_copies_all_fields(self):
        ...
        self.assertEqual(
            occurrence.google_calendar_sync_enabled,
            template.google_calendar_sync_enabled,
        )
...
class TaskAutomationBusinessLogicTest(TestCase):
    ...
    def test_task_assignment_logic(self):
        ...
        UserOffice.objects.create(
            user=admin_user, office=self.office, role="office_admin"
        )

        UserOffice.objects.create(
            user=regular_user, office=self.office, role="standard_user"
        )
        ...
        def get_assignment_suggestions(task, requester):
            ...
            # Self-assignment
            suggestions.append(
                {"user": requester, "reason": "self_assignment", "priority": 1}
            )

            # Same office users
            office_users = User.objects.filter(useroffice__office=task.office).exclude(
                id=requester.id
            )
        ...
        task = Task.objects.create(
            title="Assignment Test Task", office=self.office, created_by=self.user
        )
    ...
    def test_task_deadline_calculation(self):
        ...
        def calculate_suggested_deadline(task_type, urgency="medium"):
            ...
            urgency_multipliers = {"low": 2.0, "medium": 1.0, "high": 0.5}
