...
class TaskCreateView(LoginRequiredMixin, CreateView):
        ...
        if request.user.role != "super_admin":
            ...
            if not UserOffice.objects.filter(user_id=str(request.user.id)).exists():
                raise PermissionDenied(
                    "Access denied. User not assigned to any office."
                )
    ...
    def form_valid(self, form):
        ...
        if form.instance.is_recurring_template:
            form.instance.next_occurrence_date = (
                form.instance.calculate_next_occurrence()
            )
            form.instance.save(update_fields=["next_occurrence_date"])
            messages.success(
                self.request,
                f'Recurring task template "{form.instance.title}" created successfully.',
            )
        else:
            messages.success(
    ...
    def _sync_task_to_calendar(self, task):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"DEBUG: User {self.request.user.username} has Google token, expires: {google_token.expires_at}"
                )
            ...
            messages.warning(
                self.request, f"Task created but calendar sync failed: {str(e)}"
            )
...
class TaskUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if (
            form.cleaned_data.get("status") == "completed"
            and original_task.status != "completed"
        ):
        ...
        if task.is_recurring_template and (
            original_task.recurring_pattern != task.recurring_pattern
            or original_task.due_date != task.due_date
        ):
        ...
        if (
            task.google_calendar_sync_enabled
            and not task.google_calendar_event_id
            and not original_task.google_calendar_sync_enabled
        ):
    ...
    def _sync_task_to_calendar(self, task):
        ...
        try:
            ...
            messages.warning(
                self.request, f"Task updated but calendar sync failed: {str(e)}"
            )
...
class TaskCompleteView(LoginRequiredMixin, View):
    def post(self, request, pk):
        ...
        return JsonResponse(
            {"success": False, "message": "Task already completed or no change."},
            status=400,
        )
...
def bulk_update_task_status(request):
    ...
    try:
        ...
        messages.success(
            request, f"Successfully updated {count} tasks to {status_display}."
        )
...
def bulk_update_task_priority(request):
    ...
    try:
        ...
        messages.success(
            request,
            f"Successfully updated {count} tasks to {priority_display} priority.",
        )
...
def bulk_assign_task_user(request):
    ...
    try:
        ...
        messages.success(
            request, f"Successfully assigned {count} tasks to {user_name}."
        )
...
def bulk_assign_task_office(request):
    ...
    try:
        ...
        messages.success(
            request, f"Successfully assigned {count} tasks to {office.name}."
        )
