...
class TaskForm(forms.ModelForm):
    ...
    recurrence_interval = forms.IntegerField(
        min_value=1, initial=1, required=False, label="Repeat every"
    )
    ...
    recurrence_day_of_month = forms.IntegerField(
        min_value=1, max_value=31, required=False, label="Day of month (for monthly)"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            self.fields["is_recurring_template"].initial = (
                self.instance.is_recurring_template
            )
            self.fields["recurrence_end_date"].initial = (
                self.instance.recurrence_end_date
            )
            if self.instance.recurring_pattern:
                self.fields["recurrence_frequency"].initial = (
                    self.instance.recurring_pattern.get("frequency")
                )
                self.fields["recurrence_interval"].initial = (
                    self.instance.recurring_pattern.get("interval", 1)
                )
                self.fields["recurrence_weekdays"].initial = (
                    self.instance.recurring_pattern.get("weekdays", [])
                )
                self.fields["recurrence_day_of_month"].initial = (
                    self.instance.recurring_pattern.get("day_of_month")
                )
        ...
        self.helper.layout = Layout(
            ...
            Row(
                ...
                Column("due_time", css_class="form-group col-md-6 mb-3"),
            ),
            Row(
            ...
            Row(
                ...
                Column("office", css_class="form-group col-md-6 mb-3"),
            ),
            Row(
            ...
            Row(
                Column("reminder_option", css_class="form-group col-md-6 mb-3"),
                Column("reminder_time", css_class="form-group col-md-6 mb-3"),
            ),
            "google_calendar_sync_enabled",
    ...
    def clean(self):
        ...
        if is_recurring and not frequency:
            self.add_error(
                "recurrence_frequency", "Frequency is required for recurring tasks."
            )

        if frequency == "weekly" and not cleaned_data.get("recurrence_weekdays"):
            self.add_error(
                "recurrence_weekdays",
                "Please select at least one weekday for weekly recurrence.",
            )
    ...
    def save(self, commit=True):
        ...
        instance.is_recurring_template = self.cleaned_data.get(
        ...
        if instance.is_recurring_template:
            instance.recurring_pattern = {
                ...
                "weekdays": (
                    [int(wd) for wd in self.cleaned_data.get("recurrence_weekdays", [])]
                    if self.cleaned_data.get("recurrence_frequency") == "weekly"
                    else None
                ),
                "day_of_month": (
                    self.cleaned_data.get("recurrence_day_of_month")
                    if self.cleaned_data.get("recurrence_frequency") == "monthly"
                    else None
                ),
            }
            ...
            if (
                instance.due_date
            ):  # due_date is set on the instance by super().save(commit=False) from cleaned_data
                ...
                if due_time_value:
                    ...
                    if isinstance(due_time_value, str):
                        ...
                        time_component = datetime.strptime(
                            due_time_value, "%H:%M"
                        ).time()
                    else:
                ...
                naive_datetime = timezone.datetime.combine(
                    instance.due_date, time_component
                )
...
class TaskFilterForm(forms.Form):
    ...
    status = forms.ChoiceField(
        ...
        + Task.STATUS_CHOICES,  # Use model choices directly
        required=False,
        widget=forms.Select(attrs={"class": "form-select form-select-sm"}),
    )
    priority = forms.ChoiceField(
        choices=[("", "All Priorities")]
        + Task.PRIORITY_CHOICES,  # Use model choices directly
        required=False,
        widget=forms.Select(attrs={"class": "form-select form-select-sm"}),
    )
