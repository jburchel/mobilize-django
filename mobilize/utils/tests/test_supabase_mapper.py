...
class SupabaseMapperTestCase(TestCase):
    ...
    def test_type_conversion_to_supabase(self):
        ...
        self.assertEqual(
            SupabaseMapper.TYPE_MAPPING["Person.user_id"]["to_supabase"](5), "5"
        )
        self.assertEqual(
            SupabaseMapper.TYPE_MAPPING["office_id"]["to_supabase"](10), "10"
        )

        # Test handling None values
        self.assertIsNone(
            SupabaseMapper.TYPE_MAPPING["Person.user_id"]["to_supabase"](None)
        )
    ...
    def test_type_conversion_from_supabase(self):
        ...
        self.assertEqual(
            SupabaseMapper.TYPE_MAPPING["Person.user_id"]["from_supabase"]("5"), 5
        )
        self.assertEqual(
            SupabaseMapper.TYPE_MAPPING["office_id"]["from_supabase"]("10"), 10
        )

        # Test handling None values
        self.assertIsNone(
            SupabaseMapper.TYPE_MAPPING["Person.user_id"]["from_supabase"](None)
        )
        self.assertIsNone(
            SupabaseMapper.TYPE_MAPPING["office_id"]["from_supabase"](None)
        )

        # Test handling empty string values
        self.assertIsNone(
            SupabaseMapper.TYPE_MAPPING["Person.user_id"]["from_supabase"]("")
        )
