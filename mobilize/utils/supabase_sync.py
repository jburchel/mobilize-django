...
class SupabaseSync:
        ...
        try:
            if instance_id:
                ...
                if existing_record:
                    ...
                    logger.info(f"Updated {model_class.__name__} with ID {instance_id} in Supabase")
                else:
                    # Insert new record with specified ID
                    result = supabase.insert(model_class, supabase_data)
                    logger.info(f"Inserted {model_class.__name__} with ID {instance_id} into Supabase")
            else:
    ...
    @classmethod
    def sync_from_supabase(cls, supabase_data: Dict[str, Any], model_class: Type[models.Model]) -> Optional[models.Model]:
        """
        ...
        try:
            ...
            if model_class in [Person, Church]:
                ...
                try:
                    ...
                    contact_data = {k: v for k, v in django_data.items() if k in contact_fields}
                    ...
                    try:
                        ...
                        logger.info(f"Created Contact record with ID {contact_instance.id} for {model_class.__name__}")
                    except Exception as contact_error:
                        logger.error(f"Error creating Contact record: {str(contact_error)}")
                        ...
                        logger.info(f"Created minimal Contact record with ID {contact_instance.id}")
                
                # Now we can update or create the actual model instance
                if instance:
                    # Update existing instance
                    model_fields = [f.name for f in model_class._meta.fields if f.name != 'contact_ptr']
                    ...
                    instance.save()
                    logger.info(f"Updated {model_class.__name__} with ID {instance_id} from Supabase")
                else:
                    # Create new instance
                    try:
                        ...
                        model_fields = [f.name for f in model_class._meta.fields if f.name != 'contact_ptr']
                        model_data = {k: v for k, v in django_data.items() if k in model_fields}
                        ...
                        logger.info(f"Created new {model_class.__name__} with ID {instance.id} from Supabase")
                    except Exception as create_error:
                        logger.error(f"Error creating {model_class.__name__} with raw SQL: {str(create_error)}")
                        ...
                        try:
                            ...
                            for field_name, field_value in model_data.items():
                                if hasattr(instance, field_name):
                                    setattr(instance, field_name, field_value)
                            instance.save()
                            logger.info(f"Created new {model_class.__name__} with ID {instance.id} using fallback method")
                        except Exception as fallback_error:
                            logger.error(f"Fallback creation failed: {str(fallback_error)}")
                            return None
            else:
                ...
                with transaction.atomic():
                    if instance:
                        ...
                        try:
                            ...
                            logger.error(f"Error creating {model_class.__name__}: {str(create_error)}")
                            # Try a more selective approach with only fields that exist on the model
                            filtered_data = {k: v for k, v in django_data.items() 
                                           if k in [f.name for f in model_class._meta.fields]}
                            instance = model_class.objects.create(**filtered_data)
                            logger.info(f"Created new {model_class.__name__} with ID {instance.id} from Supabase (filtered fields)")
            
            return instance
        except Exception as e:
            logger.error(f"Error syncing {model_class.__name__} from Supabase: {str(e)}")
            return None
    ...
    @classmethod
    def bulk_sync_to_supabase(cls, instances: List[models.Model]) -> List[Dict[str, Any]]:
        """
    ...
    @classmethod
    def bulk_sync_from_supabase(cls, supabase_data_list: List[Dict[str, Any]], model_class: Type[models.Model]) -> List[models.Model]:
        """
    ...
    @classmethod
    def detect_conflicts(cls, instance: models.Model, supabase_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    @classmethod
    def resolve_conflicts(cls, instance: models.Model, conflicts: Dict[str, Dict[str, Any]], resolution_strategy: str = 'django') -> models.Model:
        """
