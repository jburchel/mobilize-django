...
class SupabaseMapper:
    ...
    def to_supabase(cls, instance: models.Model) -> Dict[str, Any]:
        ...
        for field in instance._meta.fields:
            ...
            supabase_field_name = cls.FIELD_MAPPING.get(model_name, {}).get(
            ...
            if model_field_key in cls.TYPE_MAPPING:
                field_value = cls.TYPE_MAPPING[model_field_key]["to_supabase"](
                    field_value
                )
            ...
            if hasattr(field, "related_model") and field.related_model is not None:
                ...
                if model_id_field_key in cls.TYPE_MAPPING:
                    id_value = cls.TYPE_MAPPING[model_id_field_key]["to_supabase"](
                        id_value
                    )
    ...
    @classmethod
    def from_supabase(
        cls, supabase_data: Dict[str, Any], model_class
    ) -> Dict[str, Any]:
        """
        ...
        if model_name in cls.FIELD_MAPPING:
            field_mapping_reverse = {
                v: k for k, v in cls.FIELD_MAPPING[model_name].items()
            }
        ...
        for field_name, field_value in supabase_data.items():
            ...
            if not isinstance(model_class, str):
                ...
                if not hasattr(model_class, django_field_name) and not any(
                    f.name == django_field_name for f in model_class._meta.fields
                ):
                    continue
            ...
            if model_field_key in cls.TYPE_MAPPING:
                field_value = cls.TYPE_MAPPING[model_field_key]["from_supabase"](
                    field_value
                )
            elif django_field_name in cls.TYPE_MAPPING:
                field_value = cls.TYPE_MAPPING[django_field_name]["from_supabase"](
                    field_value
                )
    ...
    @classmethod
    def bulk_to_supabase(
        cls, django_data_list: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for item in django_data_list:
            ...
            for field_name, field_value in item.items():
                ...
                if field_name in cls.TYPE_MAPPING:
                    field_value = cls.TYPE_MAPPING[field_name]["to_supabase"](
                        field_value
                    )
    ...
    @classmethod
    def bulk_from_supabase(
        cls, supabase_data_list: List[Dict[str, Any]], model_class
    ) -> List[Dict[str, Any]]:
        """
    ...
    def create_from_supabase(
        cls, data: Dict[str, Any], model_class: Type[models.Model]
    ) -> models.Model:
        """
    ...
    @classmethod
    def update_from_supabase(
        cls, instance: models.Model, data: Dict[str, Any]
    ) -> models.Model:
        """
    ...
    @classmethod
    def bulk_create_from_supabase(
        cls, data_list: List[Dict[str, Any]], model_class: Type[models.Model]
    ) -> List[models.Model]:
        """
