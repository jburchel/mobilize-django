...
def sync_supabase_task(self, direction: str = 'both', model_name: str = None, force_full_sync: bool = False):
    """
    ...
    try:
        ...
        if model_name:
            ...
            models_to_sync = ['contacts', 'persons', 'churches', 'communications', 'tasks']
        ...
        cache.set('supabase_sync_stats', sync_stats, timeout=86400)  # Cache for 24 hours
    ...
    try:
        ...
        for model in models_to_backup:
            try:
                ...
                result = sync_client.push_to_supabase(
                    model_name=model,
                    force_full=True
                )
        ...
        backup_metadata = {
            ...
            'models_backed_up': len([m for m in backup_results if 'error' not in backup_results[m]]),
            'results': backup_results,
            'backup_type': 'full_database'
        }
        
        # Cache backup status
        cache.set('last_backup_status', backup_metadata, timeout=604800)  # Cache for 1 week
...
def validate_data_integrity(self):
    ...
    try:
        ...
        invalid_emails = Contact.objects.exclude(email='').exclude(email__isnull=True).exclude(
        ...
        unassigned_tasks = Task.objects.filter(assigned_to__isnull=True, status__in=['pending', 'in_progress'])
        ...
        cache.set('data_integrity_report', integrity_report, timeout=86400)  # Cache for 24 hours
...
def optimize_database_performance(self):
    ...
    try:
        ...
        cache.set('db_optimization_report', optimization_report, timeout=86400)  # Cache for 24 hours
...
def generate_system_health_report(self):
    ...
    try:
        ...
        # Cache metrics
        health_metrics['cache_status'] = 'available' if cache.get('test_key') is None else 'available'
        
        # Recent error counts (placeholder - would need error tracking)
        health_metrics['recent_errors'] = 0  # This would be implemented with proper error tracking
        ...
        # Store health report
        cache.set('system_health_report', health_report, timeout=3600)  # Cache for 1 hour
...
def cleanup_temporary_files(self, days_old: int = 7):
    ...
    try:
        ...
        logger.info(f"Cleaned up {cleanup_results['files_removed']} files, freed {cleanup_results['space_freed_mb']} MB")
