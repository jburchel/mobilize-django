...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with connection.cursor() as cursor:
            ...
            for stage in stages:
                self.stdout.write(
                    f"- ID {stage[0]}: {stage[1]} (Pipeline {stage[2]}, Order {stage[3]})"
                )
            ...
            if wrong_table_exists:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        'Table "pipeline_contacts" exists but Django expects "pipeline_pipelinecontact"'
                    )
                )
                self.stdout.write(
                    "This is likely causing the foreign key constraint error."
                )
                ...
                if correct_table_exists:
                    self.stdout.write(
                        "Both tables exist. You may need to consolidate data."
                    )
                else:
                    self.stdout.write(
                        "Only the wrong table name exists. You may need to rename it."
                    )
