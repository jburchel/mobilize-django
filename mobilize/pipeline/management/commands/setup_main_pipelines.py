...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Created main people pipeline: {people_pipeline.name}"
                )
            )
        else:
            self.stdout.write(
                f"Main people pipeline already exists: {people_pipeline.name}"
            )

        # Create stages for people pipeline
        for order, (stage_code, stage_name) in enumerate(
            MAIN_PEOPLE_PIPELINE_STAGES, 1
        ):
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Created main church pipeline: {church_pipeline.name}"
                )
            )
        else:
            self.stdout.write(
                f"Main church pipeline already exists: {church_pipeline.name}"
            )

        # Create stages for church pipeline
        for order, (stage_code, stage_name) in enumerate(
            MAIN_CHURCH_PIPELINE_STAGES, 1
        ):
            stage, created = PipelineStage.objects.get_or_create(
        ...
        self.stdout.write(self.style.SUCCESS("Main pipelines setup completed!"))

        # Show summary
        self.stdout.write("\nPipeline Summary:")
        self.stdout.write(
            f'Main People Pipeline stages: {", ".join([s[1] for s in MAIN_PEOPLE_PIPELINE_STAGES])}'
        )
        self.stdout.write(
            f'Main Church Pipeline stages: {", ".join([s[1] for s in MAIN_CHURCH_PIPELINE_STAGES])}'
        )
        self.stdout.write(
