...
# Proxy/Load Balancer settings for Render
# This ensures Django correctly detects HTTPS when behind a proxy
...
LOGGING = {
    ...
    "root": {
        "handlers": (
            ["console", "file"]
            if os.environ.get("LOG_TO_STDOUT", "True") == "True"
            else ["file"]
        ),
        "level": os.environ.get("LOG_LEVEL", "INFO"),
    },
...
CRONJOBS = [
    ...
    (
        "0 * * * *",
        "mobilize.utils.management.commands.sync_supabase.Command.run_from_scheduler",
        ["--direction=both"],
        {},
        ">> " + str(BASE_DIR / "logs/supabase_sync.log") + " 2>&1",
    ),
    ...
    (
        ...
        ["--direction=both", "--full"],
        {},
        ">> " + str(BASE_DIR / "logs/supabase_sync_full.log") + " 2>&1",
    ),
    # Generate recurring tasks (e.g., every hour)
    (
        "0 * * * *",
        "mobilize.tasks.management.commands.generate_recurring_tasks.Command",
        [],
        {},
        ">> " + str(BASE_DIR / "logs/generate_recurring_tasks.log") + " 2>&1",
    ),
]
...
CELERY_TASK_ALWAYS_EAGER = (
    os.environ.get("CELERY_ALWAYS_EAGER", "False") == "True"
)  # For testing
CELERY_TASK_EAGER_PROPAGATES = True
CELERY_WORKER_PREFETCH_MULTIPLIER = (
    1  # Disable prefetching for better task distribution
)
