...
def lazy_image(src, alt="", css_class="", width=None, height=None, placeholder=None):
    ...
    if not placeholder:
        if width and height:
            ...
            placeholder = (
                f'data:image/svg+xml;base64,{svg_placeholder.encode("base64").decode()}'
            )
        else:
...
def add_lazy_loading(html_content):
    ...
    def replace_img(match):
        ...
        new_img_tag = img_tag.replace(
            f'src="{src}"', f'src="{placeholder}" data-lazy="{src}"'
        ).replace(f"src='{src}'", f'src="{placeholder}" data-lazy="{src}"')
