...
class ReportGenerator:
    ...
    def __init__(self, user, view_mode="default"):
        ...
        """
        self.user = user
        self.access_manager = get_data_access_manager(
            type("obj", (object,), {"user": user, "GET": {"view_mode": view_mode}})()
        )
    ...
    def generate_dashboard_summary(self, format="csv"):
        ...
        pending_tasks = (
            self.access_manager.get_tasks_queryset().filter(status="pending").count()
        )
        completed_tasks = (
            self.access_manager.get_tasks_queryset().filter(status="completed").count()
        )
    ...
    def _generate_people_csv(self, queryset):
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="people_report_{datetime.now().strftime("%Y%m%d")}.csv"'
        )
        ...
        for person in queryset:
            writer.writerow(
                [
                    ...
                    (
                        person.contact.last_contact_date.strftime("%Y-%m-%d")
                        if person.contact.last_contact_date
                        else ""
                    ),
                    (
                        person.contact.created_at.strftime("%Y-%m-%d")
    ...
    def _generate_churches_csv(self, queryset):
        ...
        response = HttpResponse(content_type="text/csv")
        response["Content-Disposition"] = (
            f'attachment; filename="churches_report_{datetime.now().strftime("%Y%m%d")}.csv"'
        )
        ...
        for church in queryset:
            writer.writerow(
                [
                    ...
                    (
                        church.contact.created_at.strftime("%Y-%m-%d")
                        if church.contact.created_at
                        else ""
                    ),
                ]
            )
    ...
    def _generate_tasks_csv(self, queryset):
        ...
        response = HttpResponse(content_type="text/csv")
        response["Content-Disposition"] = (
            f'attachment; filename="tasks_report_{datetime.now().strftime("%Y%m%d")}.csv"'
        )
    ...
    def _generate_communications_csv(self, queryset):
        ...
        response = HttpResponse(content_type="text/csv")
        response["Content-Disposition"] = (
            f'attachment; filename="communications_report_{datetime.now().strftime("%Y%m%d")}.csv"'
        )
