...
class Command(BaseCommand):
    ...
    def create_offices(self):
        ...
        offices_data = [
            {'name': 'North America Office', 'code': 'NA', 'city': 'Dallas', 'state': 'TX', 'country': 'USA'},
            {'name': 'Europe Office', 'code': 'EU', 'city': 'London', 'country': 'UK'},
            {'name': 'Asia Pacific Office', 'code': 'AP', 'city': 'Singapore', 'country': 'Singapore'},
            {'name': 'Latin America Office', 'code': 'LA', 'city': 'Mexico City', 'country': 'Mexico'},
        ]
    ...
    def create_churches(self, count, offices, users):
        ...
        church_names = [
            'First Baptist Church', 'Grace Community Church', 'New Life Fellowship',
            'Cornerstone Church', 'Harvest Christian Center', 'Faith Bible Church',
            'Trinity Methodist Church', 'Hope Presbyterian Church', 'Victory Chapel',
            'Crossroads Church', 'Lighthouse Baptist Church', 'River of Life Church',
            'Emmanuel Church', 'Calvary Chapel', 'Bethel Assembly of God',
            'Christ the King Church', 'Redeemer Lutheran Church', 'Sacred Heart Church',
            'Unity Fellowship', 'Abundant Life Church', 'Mountain View Church',
            'Valley Christian Center', 'Sunrise Baptist Church', 'Peace Lutheran Church',
            'Joy Christian Fellowship', 'Rock Church', 'Living Water Church',
            'New Hope Baptist Church', 'Grace Point Church', 'Freedom Church',
            'Restoration Church', 'The Bridge Church', 'Elevation Church',
            'Journey Church', 'Mosaic Church', 'Catalyst Church', 'Impact Church',
            'Overflow Church', 'Anchor Church', 'Compass Church', 'Refuge Church',
            'Sanctuary Church', 'Harbor Church', 'Oasis Church', 'Vineyard Church',
            'Gateway Church', 'Pathway Church', 'Summit Church', 'Horizon Church',
        ]
        
        denominations = [
            'Baptist', 'Methodist', 'Presbyterian', 'Pentecostal', 'Lutheran', 
            'Non-denominational', 'Assembly of God', 'Episcopal', 'Catholic',
            'Evangelical', 'Reformed', 'Mennonite', 'Nazarene', 'Wesleyan',
            'Church of Christ', 'Disciples of Christ', 'Adventist', 'Quaker'
        ]
        ...
        for i in range(count):
            ...
            church = Church.objects.create(
                ...
                service_times={
                    ...
                    "sunday_evening": f"{random.choice(['5:00', '5:30', '6:00', '6:30', '7:00'])} PM" if random.choice([True, False]) else None,
                    "wednesday": f"{random.choice(['6:30', '7:00', '7:30'])} PM" if random.choice([True, False]) else None,
                },
                facilities={
                    ...
                    "gym": random.choice([True, False]) if size_category in ['medium', 'large', 'mega'] else False,
                },
                ministries=[
                    random.choice(['Youth Ministry', 'Children\'s Ministry', 'Women\'s Ministry', 'Men\'s Ministry']),
                    random.choice(['Worship Ministry', 'Missions Ministry', 'Small Groups', 'Outreach Ministry']),
                    random.choice(['Senior Ministry', 'Young Adults', 'Marriage Ministry', 'Recovery Ministry']),
                ][:random.randint(2, 4)],
                primary_language='English',
                secondary_languages=[random.choice(['Spanish', 'Korean', 'Chinese', 'Portuguese', 'French'])] if random.choice([True, False, False]) else None,
    ...
    def create_people(self, count, offices, users):
        ...
        for i in range(count):
            ...
            person = Person.objects.create(
                ...
                home_country='USA',
                languages=['English'] if random.choice([True, False, False]) else ['English', random.choice(['Spanish', 'French', 'German', 'Korean', 'Chinese'])],
                linkedin_url=f"https://linkedin.com/in/{first_name.lower()}{last_name.lower()}" if random.choice([True, False, False]) else None,
            )
    ...
    def create_church_memberships(self, people, churches):
        ...
        for church in churches:
            ...
            available_people = [p for p in people if not p.church_memberships.filter(church=church).exists()]
            if available_people:
                ...
                if is_pastor:
                    role = random.choice(['senior_pastor', 'associate_pastor', 'missions_pastor'])
                else:
                    role = random.choice(['elder', 'deacon', 'board_member', 'ministry_leader', 'secretary'])
        ...
        people_to_assign = random.sample(remaining_people, min(int(len(people) * 0.6), len(remaining_people)))
        
        for person in people_to_assign:
            church = random.choice(churches)
            
            # Don't create duplicate memberships
            if not ChurchMembership.objects.filter(person=person, church=church).exists():
    ...
    def update_church_primary_contacts(self, churches):
        ...
        self.stdout.write(f'Updated primary contact information for {updated_count} churches')
    ...
    def create_tasks(self, people, churches, users, offices):
        ...
        task_titles = [
            'Follow up on visit', 'Send welcome package', 'Schedule meeting',
            'Prepare presentation', 'Review partnership proposal', 'Make phone call',
            'Send thank you note', 'Update contact information', 'Plan event',
            'Coordinate volunteers', 'Prepare materials', 'Schedule training',
            'Send partnership agreement', 'Organize mission trip', 'Plan fundraising event',
            'Coordinate youth program', 'Schedule pastor meeting', 'Prepare ministry materials',
            'Follow up on donation', 'Plan outreach event', 'Coordinate volunteer training',
            'Send quarterly report', 'Schedule church visit', 'Prepare budget proposal',
            'Organize leadership retreat', 'Plan community service', 'Coordinate worship team',
            'Send ministry update', 'Schedule board meeting', 'Plan evangelism training',
            'Coordinate missions conference', 'Prepare annual report', 'Plan stewardship campaign',
        ]
        ...
        for i in range(150):  # Create 150 tasks
            ...
            task = Task.objects.create(
                ...
                office=random.choice(offices),
                completed_at=fake.date_time_between(start_date=due_date, end_date='now') if status == 'completed' else None,
            )
            tasks.append(task)
    ...
    def create_communications(self, people, churches, users, offices):
        ...
        subjects = [
            'Partnership Discussion', 'Follow-up Meeting', 'Thank You',
            'Event Invitation', 'Resource Sharing', 'Prayer Request',
            'Volunteer Opportunity', 'Ministry Update', 'Support Request',
            'Mission Trip Information', 'Youth Program Details', 'Fundraising Campaign',
            'Leadership Training', 'Community Outreach', 'Worship Service Planning',
            'Budget Discussion', 'Facility Needs', 'Staff Meeting',
            'Evangelism Training', 'Discipleship Program', 'Small Group Coordination',
            'Missions Conference', 'Stewardship Teaching', 'Church Planting',
            'Pastoral Care', 'Counseling Services', 'Marriage Ministry',
            'Children\'s Ministry', 'Senior Adult Ministry', 'Recovery Program',
        ]
    ...
    def setup_pipeline_data(self, people, churches):
        ...
        for person in people:
            ...
            PipelineContact.objects.get_or_create(
                ...
                current_stage=stage,
                contact_type='person',
                defaults={'entered_at': fake.date_time_between(start_date='-90d', end_date='now')}
            )
