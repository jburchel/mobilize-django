...
class Command(BaseCommand):
    ...
    def review_email_duplicates(self, limit, auto_merge):
        ...
        email_duplicates = Contact.objects.values('email').annotate(
            count=Count('id')
        ...
        for i, dup in enumerate(email_duplicates[:limit]):
            ...
            if self.is_obvious_email_duplicate(contacts):
                self.stdout.write(self.style.SUCCESS('     → OBVIOUS DUPLICATE (same email, similar data)'))
                if auto_merge:
    ...
    def review_name_duplicates(self, limit, auto_merge):
        ...
        name_duplicates = Contact.objects.filter(type='person').values(
        ...
        for i, dup in enumerate(name_duplicates[:limit]):
            ...
            for j, contact in enumerate(contacts):
                ...
                self.stdout.write(f'     Phone: {contact.phone or "None"}')
                self.stdout.write(f'     Office: {contact.office}')
                self.stdout.write(f'     Notes: {(contact.notes or "None")[:50]}{"..." if contact.notes and len(contact.notes) > 50 else ""}')
            
            # Check if it's an obvious merge case
            recommendation = self.analyze_name_duplicates(contacts)
            if recommendation['obvious']:
                self.stdout.write(self.style.SUCCESS(f'     → OBVIOUS DUPLICATE: {recommendation["reason"]}'))
                if auto_merge:
                    self.auto_merge_contacts(contacts, f"name: {first} {last}")
                else:
                    self.suggest_merge(contacts, recommendation['keep_id'])
            else:
                self.stdout.write(self.style.WARNING(f'     → NEEDS MANUAL REVIEW: {recommendation["reason"]}'))

    def review_church_duplicates(self, limit, auto_merge):
        """Review church duplicates"""
        self.stdout.write('=== CHURCH DUPLICATES REVIEW ===')
        
        church_duplicates = Contact.objects.filter(type='church').values(
            'church_name'
        ).annotate(count=Count('id')).filter(
            count__gt=1,
        ...
        for i, dup in enumerate(church_duplicates[:limit]):
            ...
            for j, contact in enumerate(contacts):
                ...
                self.stdout.write(f'     Address: {contact.city or "None"}, {contact.state or "None"}')
    ...
    def analyze_church_duplicates(self, contacts):
        ...
        elif (contact1.office and contact2.office and 
              contact1.office.name != contact2.office.name):
            return {
                'obvious': False, 
