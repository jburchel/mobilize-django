        ...
        try:
            with connection.cursor() as cursor:
                ...
                for col_name, col_type, nullable in current_columns:
                    self.stdout.write(
                        f"   {col_name}: {col_type} ({'NULL' if nullable == 'YES' else 'NOT NULL'})"
                    )

                # Check if we have the wrong schema (id instead of contact_id)
                has_id_column = any(col[0] == "id" for col in current_columns)
                has_contact_id_column = any(
                    col[0] == "contact_id" for col in current_columns
                )

                self.stdout.write(f"📊 Has 'id' column: {has_id_column}")
                self.stdout.write(
                    f"📊 Has 'contact_id' column: {has_contact_id_column}"
                )

                if has_id_column and not has_contact_id_column:
                    self.stdout.write(
                        "🔧 SCHEMA MISMATCH DETECTED: Production has 'id' but should have 'contact_id'"
                    )
                    ...
                    if row_count > 0:
                        self.stdout.write(
                            "⚠️  Table has data - need to migrate carefully"
                        )
                        ...
                        self.stdout.write(
                            "📋 Sample data check (people.id vs contacts.id):"
                        )
                        matches = 0
                        for p_id, c_id, fname, lname in sample_data:
                            match = "✅" if p_id == c_id else "❌"
                            self.stdout.write(
                                f"   {match} people.id={p_id}, contact.id={c_id}, name={fname} {lname}"
                            )
                            if p_id == c_id:
                                matches += 1
                        ...
                        if matches == len(sample_data) and len(sample_data) > 0:
                            self.stdout.write(
                                "✅ Data alignment verified - safe to rename column"
                            )

                            # Step 1: Rename id to contact_id
                            self.stdout.write(
                                "🔧 Renaming 'id' column to 'contact_id'..."
                            )
                            cursor.execute(
                            ...
                            for column_def in missing_columns:
                                try:
                                    cursor.execute(
                                        f"ALTER TABLE people ADD COLUMN IF NOT EXISTS {column_def};"
                                    )
                                    self.stdout.write(
                                        f"   ✅ Added column: {column_def}"
                                    )
                                except Exception as e:
                                    self.stdout.write(
                                        f"   ⚠️  Column may exist: {column_def} - {e}"
                                    )
                            ...
                            legacy_columns = [
                                "church_role",
                                "church_id",
                            ]  # These moved to ChurchMembership
                            ...
                            self.stdout.write(
                                "❌ Data alignment mismatch - manual intervention required"
                            )
                            return
                    else:
                    ...
                    self.stdout.write(
                        "✅ People table schema is correct (has contact_id)"
                    )

                else:
                    self.stdout.write(
                        "❌ People table has neither 'id' nor 'contact_id' - unexpected state"
                    )
