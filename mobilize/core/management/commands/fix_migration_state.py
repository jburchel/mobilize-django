...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            ...
            self.stdout.write(
                f"📊 Found existing columns in people table: {existing_columns}"
            )

            if (
                "desired_service" in existing_columns
                and "info_given" in existing_columns
            ):
                self.stdout.write(
                    self.style.WARNING(
                ...
                if not dry_run:
                    ...
                    self.stdout.write(
                        "🔄 Marking migration 0008_add_person_mission_fields as fake applied..."
                    )
