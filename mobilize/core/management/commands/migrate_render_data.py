    ...
    def create_assignment_mapping(self):
        ...
        try:
            ...
            if matthew_user:
                ...
                self.stdout.write(self.style.WARNING('   Matthew Rule user not found in any expected email, using Jim Burchel as default for churches'))
                self.stdout.write(self.style.WARNING('   NOTE: This suggests script is running against local database, not production'))
            ...
            self.stdout.write(self.style.ERROR('   Please ensure j.burchel@crossoverglobal.net user exists'))
            raise
                ...
                if supabase_contact:
                    ...
                    if church_obj:
                        ...
                        if church_updates and not self.dry_run:
                            ...
                            safe_church_updates = {k: v for k, v in church_updates.items() 
                                                 if k not in ['name']}
                            if safe_church_updates:
                                for field, value in safe_church_updates.items():
                                    setattr(church_obj, field, value)
                                church_obj.save(update_fields=list(safe_church_updates.keys()))
                                updates_made = True
                ...
                if supabase_contact:
                    ...
                    try:
                        ...
                        if person_updates and not self.dry_run:
                            # Filter out problematic fields that might trigger database functions
                            safe_person_updates = {k: v for k, v in person_updates.items() 
                            ...
                            if safe_person_updates:
                                ...
                                person_obj.save(update_fields=list(safe_person_updates.keys()))
                                updates_made = True
