...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            # Migrate church data
            church_stats = self.migrate_church_data(
        ...
        try:
            ...
            for email in possible_emails:
                try:
                    ...
                    self.stdout.write(f"   Matthew Rule not found at: {email}")
                    continue
            ...
            if matthew_user:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        "   Matthew Rule user not found in any expected email, using Jim Burchel as default for churches"
                    )
                )
                self.stdout.write(
                    self.style.WARNING(
                        "   NOTE: This suggests script is running against local database, not production"
                    )
                )
            ...
            self.stdout.write(
                f"   Default church user: {default_church_user.get_full_name()} ({default_church_user.email})"
            )
            ...
            self.stdout.write(
                self.style.ERROR(
                    "   Please ensure j.burchel@crossoverglobal.net user exists"
                )
            )
            raise
        ...
        for render_church in render_churches:
            ...
            try:
                ...
                if supabase_contact:
                        ...
                        if assigned_to != "Unassigned":
                            ...
                            if assigned_to in assignment_mapping:
                                ...
                                if not supabase_contact.user:
                                    ...
                                    updates_made = True
                                    self.stdout.write(
                                        f"   Mapped {assigned_to} → {mapped_user.get_full_name()}"
                                    )
                            else:
                                ...
                                field_updates["notes"] = self.append_to_notes(
                                    supabase_contact.notes, assignment_note
                                )
                                updates_made = True

                                if self.dry_run:
                                    self.stdout.write(
                                        f"   Would assign {render_church['church_name']} to m.rule (was: {assigned_to})"
                                    )

                    # Update other missing contact fields
                    if (
                        render_church["initial_notes"]
                        and not supabase_contact.initial_notes
                    ):
                    ...
                    if (
                        render_church["info_given"]
                        and render_church["info_given"].strip()
                    ):
                    ...
                    if church_obj:
                        ...
                        if (
                            ...
                            or render_church["senior_pastor_last_name"]
                        ):
                        ...
                        if render_church["senior_pastor_phone"]:
                            pastor_info.append(
                                f"Pastor Phone: {render_church['senior_pastor_phone']}"
                            )

                        if render_church["senior_pastor_email"]:
                            pastor_info.append(
                                f"Pastor Email: {render_church['senior_pastor_email']}"
                            )
                        ...
                        if pastor_info:
                            ...
                            if not church_obj.info_given:
                                ...
                                church_updates["info_given"] = (
                                    f"{church_obj.info_given}; {pastor_text}"
                                )

                        if church_updates and not self.dry_run:
                            # Filter out problematic fields that might trigger database functions
                            safe_church_updates = {
                                k: v
                                for k, v in church_updates.items()
                                if k not in ["name"]
                            }
                            if safe_church_updates:
                                for field, value in safe_church_updates.items():
                                    setattr(church_obj, field, value)
                                church_obj.save(
                                    update_fields=list(safe_church_updates.keys())
                                )
                                updates_made = True
                    ...
                    if render_church["church_pipeline"] and render_church[
                        ...
                        self.migrate_pipeline_stage(
                            supabase_contact, render_church["church_pipeline"], "church"
                        )
                        updates_made = True

                    # Apply contact updates (avoid name fields that trigger recursion)
                    if field_updates and not self.dry_run:
                        # Filter out problematic fields that trigger database functions
                        safe_updates = {
                            k: v
                    ...
                    if updates_made:
                        ...
                        if processed % 10 == 0 or self.dry_run:
                            ...
                            if "user" in field_updates:
                                update_info.append(
                                    f"assigned to {field_updates['user'].get_full_name()}"
                                )
                            ...
                            self.stdout.write(
                                f"   {action} {supabase_contact.church_name}: {', '.join(update_info)}"
                            )
        ...
        for render_person in render_people:
            ...
            try:
                ...
                if supabase_contact:
                    ...
                    if (
                        render_person["info_given"]
                        and render_person["info_given"].strip()
                    ):
                    ...
                    try:
                        ...
                        if person_updates and not self.dry_run:
                            ...
                            if safe_person_updates:
                                for field, value in safe_person_updates.items():
                                    setattr(person_obj, field, value)
                                person_obj.save(
                                    update_fields=list(safe_person_updates.keys())
                                )
                                updates_made = True
                    ...
                    if render_person["people_pipeline"] and render_person[
                        ...
                        self.migrate_pipeline_stage(
                            supabase_contact, render_person["people_pipeline"], "person"
                        )
                        updates_made = True
