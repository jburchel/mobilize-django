        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                for contact_id, title, profession, organization in raw_samples:
                    self.stdout.write(f"   contact_id={contact_id}, title={title}, profession={profession}, org={organization}")
            ...
            try:
                ...
                if first_person:
                    self.stdout.write(f"2. First person: PK={first_person.pk}, Contact ID={first_person.contact_id}")
                    self.stdout.write(f"   Has ID attr: {hasattr(first_person, 'id')}")
                    
                    # Test contact access
                    try:
                        contact = first_person.contact
                        self.stdout.write(f"   Contact: {contact.first_name} {contact.last_name}")
                    except Exception as e:
                        self.stdout.write(f"   ❌ Error accessing contact: {e}")
                else:
                    self.stdout.write(f"2. ❌ No first person found despite count={basic_count}")
            except Exception as e:
            ...
            try:
                ...
                self.stdout.write(f"   Converted to list: {len(no_prefetch_list)} items")
            ...
            try:
                ...
                self.stdout.write(f"9. Only select_related('contact'): {basic_related_count}")
                
                basic_related_list = list(people_basic_related[:3])
                self.stdout.write(f"   Converted to list: {len(basic_related_list)} items")
