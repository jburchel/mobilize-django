...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            assignment_mapping, default_church_user, default_office = (
                self.create_assignment_mapping()
            )

            # Migrate missing people first
            people_stats = self.migrate_missing_people(
                render_conn, assignment_mapping, default_office
            )

            # Migrate missing churches
            church_stats = self.migrate_missing_churches(
                render_conn, assignment_mapping, default_church_user, default_office
            )

            # Update existing contacts with missing data (unless --only-missing)
            if not self.only_missing:
                existing_people_stats = self.update_existing_people(
                    render_conn, assignment_mapping
                )
                existing_church_stats = self.update_existing_churches(
                    render_conn, assignment_mapping, default_church_user
                )
            else:
                existing_people_stats = {"processed": 0, "updated": 0}
                existing_church_stats = {
                    "processed": 0,
                    "updated": 0,
                    "assigned_to_mrule": 0,
                }
            ...
            self.stdout.write(
                f"Churches assigned to m.rule: {church_stats['assigned_to_mrule']}"
            )

            if not self.only_missing:
                self.stdout.write("\\n--- EXISTING CONTACTS UPDATED ---")
                self.stdout.write(
                    f"Existing people updated: {existing_people_stats['updated']}"
                )
                self.stdout.write(
                    f"Existing churches updated: {existing_church_stats['updated']}"
                )
            ...
            self.stdout.write(
                f"Total churches in Supabase: {church_stats['total_in_supabase']}"
            )
    ...
    def create_assignment_mapping(self):
        ...
        try:
            ...
            matthew_user = None
            possible_emails = [
                "m.rule@crossoverglobal.net",
                "matthew.rule@crossoverglobal.net",
                "mrule@crossoverglobal.net",
            ]
            ...
            if matthew_user:
                ...
                self.stdout.write(
                    f"   Using Matthew Rule as default church user: {matthew_user.email}"
                )
            else:
                # If Matthew Rule doesn't exist, use Jim Burchel as default
                self.stdout.write(
                    self.style.WARNING(
                        "   Matthew Rule user not found, using Jim Burchel as default for churches"
                    )
                )
            ...
            self.stdout.write(
                self.style.ERROR(
                    f"   Could not find required user j.burchel@crossoverglobal.net: {e}"
                )
            )
            raise
    ...
    def check_duplicate_person(self, render_person):
        ...
        if render_person["email"]:
            existing = Contact.objects.filter(
                type="person", email=render_person["email"]
            ).first()
            if existing:
    ...
    def check_duplicate_church(self, render_church):
        ...
        if render_church["church_name"]:
            ...
            existing = Contact.objects.filter(
                type="church", church_name__iexact=render_church["church_name"]
            ).first()
            if existing:
            ...
            # Try contains match
            existing = Contact.objects.filter(
                type="church", church_name__icontains=render_church["church_name"]
            ).first()
            if existing:
        ...
        if render_church["email"]:
            existing = Contact.objects.filter(
                type="church", email=render_church["email"]
            ).first()
            if existing:
                return existing, "email"
    ...
    def migrate_missing_people(self, render_conn, assignment_mapping, default_office):
        ...
        for render_person in render_people:
            try:
                ...
                existing_contact, match_type = self.check_duplicate_person(
                    render_person
                )
                ...
                if existing_contact:
                    ...
                    if self.dry_run:
                        self.stdout.write(
                            f"   Would skip {render_person['first_name']} {render_person['last_name']} (exists: {match_type})"
                        )
                    continue
                ...
                if not self.dry_run:
                    with transaction.atomic():
                        ...
                        # Handle assignment
                        if (
                            render_person["assigned_to"]
                            and render_person["assigned_to"].strip()
                            and render_person["assigned_to"] != "Unassigned"
                        ):
                            ...
                            if assigned_to in assignment_mapping:
                                ...
                                # Add assignment note
                                assignment_note = self.create_assignment_note(
                                    assigned_to, "person"
                                )
                                contact.notes = self.append_to_notes(
                                    contact.notes, assignment_note
                                )
                        ...
                        # Handle pipeline
                        if render_person["people_pipeline"] and render_person[
                            "people_pipeline"
                        ] not in ["None", "", "null"]:
                            self.migrate_pipeline_stage(
                                contact, render_person["people_pipeline"], "person"
                            )
                ...
                if created % 10 == 0 or self.dry_run:
                    ...
                    self.stdout.write(
                        f"   {action} {render_person['first_name']} {render_person['last_name']}"
                    )

            except Exception as e:
                self.stdout.write(
                    self.style.WARNING(
                        f"   Error processing {render_person['first_name']} {render_person['last_name']}: {e}"
                    )
                )
    ...
    def migrate_missing_churches(
        self, render_conn, assignment_mapping, default_church_user, default_office
    ):
        ...
        for render_church in render_churches:
            try:
                # Check for duplicates
                existing_contact, match_type = self.check_duplicate_church(
                    render_church
                )
                ...
                if existing_contact:
                    ...
                    if self.dry_run:
                        self.stdout.write(
                            f"   Would skip {render_church['church_name']} (exists: {match_type})"
                        )
                    continue
                ...
                if not self.dry_run:
                    with transaction.atomic():
                        ...
                        if (
                            render_church["assigned_to"]
                            and render_church["assigned_to"].strip()
                            and render_church["assigned_to"] != "Unassigned"
                        ):
                        ...
                        church = Church.objects.create(
                            ...
                            senior_pastor_first_name=render_church[
                                "senior_pastor_first_name"
                            ],
                            senior_pastor_last_name=render_church[
                        ...
                        # Handle pipeline
                        if render_church["church_pipeline"] and render_church[
                            "church_pipeline"
                        ] not in ["None", "", "null"]:
                            self.migrate_pipeline_stage(
                                contact, render_church["church_pipeline"], "church"
                            )
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"   Error processing {render_church['church_name']}: {e}"
                    )
                )
        ...
        for render_person in render_people:
            ...
            try:
                ...
                if existing_contact:
                    ...
                    if (
                        render_person["initial_notes"]
                        and not existing_contact.initial_notes
                    ):
                    ...
                    if (
                        ...
                        assigned_to = render_person["assigned_to"].strip()
                        if (
                            assigned_to in assignment_mapping
                            and not existing_contact.user
                        ):
                            ...
                            field_updates["notes"] = self.append_to_notes(
                                existing_contact.notes, assignment_note
                            )
                            updates_made = True
                    ...
                    try:
                        ...
                        if (
                            render_person["home_country"]
                            and not person_obj.home_country
                        ):
                            person_updates["home_country"] = render_person[
                                "home_country"
                            ]

                        if (
                            render_person["marital_status"]
                            and not person_obj.marital_status
                        ):
                            person_updates["marital_status"] = render_person[
                                "marital_status"
                            ]

                        if (
                            render_person["spouse_first_name"]
                            and not person_obj.spouse_first_name
                        ):
                            person_updates["spouse_first_name"] = render_person[
                                "spouse_first_name"
                            ]

                        if (
                            render_person["spouse_last_name"]
                            and not person_obj.spouse_last_name
                        ):
                            person_updates["spouse_last_name"] = render_person[
                                "spouse_last_name"
                            ]
                        ...
                        if (
                            render_person["desired_service"]
                            and not person_obj.desired_service
                        ):
                            person_updates["desired_service"] = render_person[
                                "desired_service"
                            ]
                    ...
                    if render_person["people_pipeline"] and render_person[
                        ...
                        self.migrate_pipeline_stage(
                            existing_contact, render_person["people_pipeline"], "person"
                        )
                        updates_made = True
    ...
    def update_existing_churches(
        self, render_conn, assignment_mapping, default_church_user
    ):
                ...
                if existing_contact:
                    ...
                    if (
                        ...
                        if (
                            ...
                            assigned_to not in assignment_mapping
                            and not existing_contact.user
                        ):
                    ...
                    try:
                        ...
                        if (
                            render_church["denomination"]
                            and not church_obj.denomination
                        ):
                            church_updates["denomination"] = render_church[
                                "denomination"
                            ]
                        ...
                        if (
                            render_church["congregation_size"]
                            and not church_obj.congregation_size
                        ):
                            church_updates["congregation_size"] = render_church[
                                "congregation_size"
                            ]

                        if (
                            render_church["year_founded"]
                            and not church_obj.year_founded
                        ):
                            church_updates["year_founded"] = render_church[
                                "year_founded"
                            ]
                        ...
                        # Pastor information
                        if (
                            render_church["senior_pastor_first_name"]
                            and not church_obj.senior_pastor_first_name
                        ):
                            church_updates["senior_pastor_first_name"] = render_church[
                                "senior_pastor_first_name"
                            ]

                        if (
                            render_church["senior_pastor_last_name"]
                            and not church_obj.senior_pastor_last_name
                        ):
                            church_updates["senior_pastor_last_name"] = render_church[
                                "senior_pastor_last_name"
                            ]

                        if (
                            render_church["senior_pastor_phone"]
                            and not church_obj.pastor_phone
                        ):
                            church_updates["pastor_phone"] = render_church[
                                "senior_pastor_phone"
                            ]

                        if (
                            render_church["senior_pastor_email"]
                            and not church_obj.pastor_email
                        ):
                            church_updates["pastor_email"] = render_church[
                                "senior_pastor_email"
                            ]
                    ...
                    if render_church["church_pipeline"] and render_church[
                        ...
                        self.migrate_pipeline_stage(
                            existing_contact, render_church["church_pipeline"], "church"
                        )
                        updates_made = True
                    ...
                    if field_updates and not self.dry_run:
                        safe_updates = {
                            ...
                            if k not in ["first_name", "last_name", "church_name"]
                        }
                        if safe_updates:
                            for field, value in safe_updates.items():
                                setattr(existing_contact, field, value)
                            existing_contact.save(
                                update_fields=list(safe_updates.keys())
                            )
                    ...
                    if updates_made:
                        ...
                        if processed % 10 == 0 or self.dry_run:
                            action = "Would update" if self.dry_run else "Updated"
                            self.stdout.write(
                                f"   {action} {existing_contact.church_name}"
                            )
    ...
    def migrate_pipeline_stage(self, contact, pipeline_stage, contact_type):
        ...
        try:
            ...
            mapped_stage = stage_mapping.get(
                pipeline_stage.upper(), pipeline_stage.title()
            )
            ...
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"   Pipeline stage '{mapped_stage}' not found for {contact_type}"
                    )
                )
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"   Error setting pipeline stage for {contact}: {e}"
                )
            )
