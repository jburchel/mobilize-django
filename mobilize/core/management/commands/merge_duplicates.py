...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["merge_contact_ids"]:
            ...
            contact_ids = [
                int(id.strip()) for id in options["merge_contact_ids"].split(",")
            ]
            keep_id = options["keep_contact_id"]
            if not keep_id:
                self.stdout.write(
                    self.style.ERROR(
                        "--keep-contact-id is required when merging specific contacts"
                    )
                )
                return
    ...
    def merge_contacts(self, contacts, reason):
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write(
                    f"  ✓ Successfully merged {len(duplicate_contacts)} duplicates"
                )
                return True
        ...
        for duplicate in duplicates:
            ...
            try:
                ...
                if duplicate.type == "person":
                    Communication.objects.filter(
                        person=duplicate.person_details
                    ).update(person=primary.person_details)
                elif duplicate.type == "church":
                    Communication.objects.filter(
                        church=duplicate.church_details
                    ).update(church=primary.church_details)
            except Exception as e:
            ...
            if duplicate.type == "person":
                try:
                    ...
                    primary_person, created = Person.objects.get_or_create(
                        contact=primary
                    )
                    ...
                    for field in [
                        ...
                        if not getattr(primary_person, field, None) and getattr(
                            dup_person, field, None
                        ):
            ...
            # For churches, merge Church details
                ...
                try:
                    ...
                    primary_church, created = Church.objects.get_or_create(
                        contact=primary, defaults={"name": primary.church_name}
                    )
                    ...
                    for field in [
                        ...
                        if not getattr(primary_church, field, None) and getattr(
                            dup_church, field, None
                        ):
    ...
    def merge_specific_contacts(self, contact_ids, keep_id):
        ...
        if keep_id not in contact_ids:
            self.stdout.write(
                self.style.ERROR(f"Keep ID {keep_id} not in contact list")
            )
            return
        ...
        for contact_id in contact_ids:
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"Contact ID {contact_id} not found")
                )
        ...
        if not primary:
            self.stdout.write(
                self.style.ERROR(f"Contact to keep (ID {keep_id}) not found")
            )
            return
