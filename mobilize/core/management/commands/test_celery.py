    ...
    def handle(self, *args, **options):
        ...
        try:
            if task_type == "debug":
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Email processing task queued with ID: {result.id}"
                    )
                )
                ...
                self.stdout.write(self.style.ERROR(f"Unknown task type: {task_type}"))
                return

            self.stdout.write("Task queued successfully!")
            self.stdout.write(
                "Note: You need Redis running and a Celery worker to process the task."
            )
            self.stdout.write(
                "Start a worker with: celery -A mobilize worker --loglevel=info"
            )

        except Exception as e:
            self.stdout.write(self.style.ERROR(f"Error queuing task: {str(e)}"))
