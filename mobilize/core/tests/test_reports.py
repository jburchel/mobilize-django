...
class ReportGeneratorTests(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="testuser", email="test@example.com", role="super_admin"
        )
    ...
    def test_tasks_report_with_status_filter(self):
        ...
        response = self.report_generator.generate_tasks_report(
            format="csv", status_filter="pending"
        )
        content = response.content.decode("utf-8")
        ...
        response = self.report_generator.generate_tasks_report(
            format="csv", status_filter="completed"
        )
        content = response.content.decode("utf-8")
    ...
    def test_tasks_report_overdue_filter(self):
        ...
        response = self.report_generator.generate_tasks_report(
            format="csv", status_filter="overdue"
        )
        content = response.content.decode("utf-8")
    ...
    def test_report_generator_with_view_mode(self):
        ...
        other_user = User.objects.create_user(
            username="otheruser", email="other@example.com", role="standard_user"
        )
