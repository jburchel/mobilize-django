...
class ActivityLogTests(TestCase):
    ...
    def test_activity_log_action_choices(self):
        ...
        for action in actions:
            log = ActivityLog.objects.create(
                user=self.user, action_type=action, entity_type="test", entity_id=1
            )
    ...
    def test_activity_log_without_user(self):
        ...
        log = ActivityLog.objects.create(
            action_type="sync", entity_type="system", details={"source": "supabase"}
        )
    ...
    def test_activity_log_ordering(self):
        ...
        log1 = ActivityLog.objects.create(
            user=self.user, action_type="create", entity_type="test1"
        )

        log2 = ActivityLog.objects.create(
            user=self.user, action_type="create", entity_type="test2"
        )
