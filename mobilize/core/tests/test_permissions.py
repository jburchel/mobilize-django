...
from mobilize.core.permissions import (
    DataAccessManager,
...
class DataAccessManagerTests(TestCase):
    ...
    def setUp(self):
        ...
        self.super_admin = User.objects.create_user(
            username="superadmin", email="super@example.com", role="super_admin"
        )

        self.office_admin = User.objects.create_user(
            username="officeadmin", email="office@example.com", role="office_admin"
        )

        self.standard_user = User.objects.create_user(
            username="standarduser", email="standard@example.com", role="standard_user"
        )

        self.limited_user = User.objects.create_user(
            username="limiteduser", email="limited@example.com", role="limited_user"
        )

        # Create user-office relationships
        UserOffice.objects.create(
            user_id=str(self.office_admin.id), office_id=self.office1.id
        )
        UserOffice.objects.create(
            user_id=str(self.standard_user.id), office_id=self.office1.id
        )
        ...
        self.church1 = Church.objects.create(
            name="Test Church 1", denomination="Baptist", office_id=self.office1.id
        )

        self.church2 = Church.objects.create(
            name="Test Church 2", denomination="Methodist", office_id=self.office2.id
        )
    ...
    def test_get_view_mode_display(self):
        ...
        self.assertEqual(
            super_admin_my_manager.get_view_mode_display(), "My Contacts Only"
        )
...
class RequireRoleDecoratorTests(TestCase):
    ...
    def setUp(self):
        ...
        self.standard_user = User.objects.create_user(
            username="standard", email="standard@example.com", role="standard_user"
        )
