#!/usr/bin/env python3
"""
Create comprehensive migration script to restore missing CRM data from Render to Supabase
"""

import os

def create_migration_script():
    """Create the Django management command for data migration"""
    
    migration_script = '''"""
Migrate missing CRM data from Render database to Supabase
...
class Command(BaseCommand):
    help = 'Migrate missing CRM data from Render database to Supabase'
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            assignment_mapping = self.create_assignment_mapping()
            
            # Add legacy_assigned_to field if it doesn't exist
            self.add_legacy_fields()
            
            # Migrate church data
            church_stats = self.migrate_church_data(render_conn, assignment_mapping)
    ...
    def create_assignment_mapping(self):
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING(f'   Could not find user: {e}'))
        
        return mapping

    def add_legacy_fields(self):
        """Add legacy_assigned_to field to contacts table if it doesn't exist"""
        if self.dry_run:
            self.stdout.write('Would add legacy_assigned_to field to contacts table')
            return
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                ...
                WHERE table_name='contacts' AND column_name='legacy_assigned_to'
            """)
            
            if not cursor.fetchone():
                self.stdout.write('Adding legacy_assigned_to field to contacts table...')
                cursor.execute("""
                    ALTER TABLE contacts 
                    ADD COLUMN legacy_assigned_to VARCHAR(255)
                """)
                self.stdout.write('   ✓ Added legacy_assigned_to field')
            else:
                self.stdout.write('   legacy_assigned_to field already exists')

    def migrate_church_data(self, render_conn, assignment_mapping):
        ...
        for render_church in render_churches:
            ...
            try:
                ...
                if supabase_contact:
                    ...
                    # Legacy assignment tracking
                    if render_church['assigned_to']:
                        field_updates['legacy_assigned_to'] = render_church['assigned_to']
                        updates_made = True
                        
                        # Map to actual user if possible
                        if render_church['assigned_to'] in assignment_mapping:
                            user = assignment_mapping[render_church['assigned_to']]
                            if not supabase_contact.user:
                                field_updates['user'] = user
                                updates_made = True
                    ...
                    if church_obj:
                        ...
                        # Pastor information - store in info_given if not present
                        pastor_info = []
                        ...
                        if pastor_info and not church_obj.info_given:
                            church_updates['info_given'] = "; ".join(pastor_info)
                        elif pastor_info and church_obj.info_given:
                            # Append to existing info
                            additional_info = "; ".join(pastor_info)
                            if additional_info not in church_obj.info_given:
                                church_updates['info_given'] = f"{church_obj.info_given}; {additional_info}"
                    ...
                    if render_church['church_pipeline'] and render_church['church_pipeline'] != 'None':
                    ...
                    if updates_made:
                        ...
                        if processed % 10 == 0 or self.dry_run:
                            ...
                            self.stdout.write(f"   {action} {supabase_contact.church_name}: {', '.join(field_updates.keys())}")
        ...
        for render_person in render_people:
            ...
            try:
                ...
                if supabase_contact:
                    ...
                    # Legacy assignment tracking
                    if render_person['assigned_to']:
                        field_updates['legacy_assigned_to'] = render_person['assigned_to']
                        updates_made = True
                        
                        # Map to actual user if possible
                        if render_person['assigned_to'] in assignment_mapping:
                            user = assignment_mapping[render_person['assigned_to']]
                            if not supabase_contact.user:
                                field_updates['user'] = user
                                updates_made = True
                    
                    # Handle additional info_given data
                    if render_person['info_given']:
                        if not supabase_contact.notes:
                            field_updates['notes'] = f"Legacy Info: {render_person['info_given']}"
                        elif render_person['info_given'] not in supabase_contact.notes:
                            field_updates['notes'] = f"{supabase_contact.notes}\\n\\nLegacy Info: {render_person['info_given']}"
                        updates_made = True
                    ...
                    try:
                        ...
                        self.stdout.write(self.style.WARNING(f"   Could not update person details for {render_person['first_name']} {render_person['last_name']}: {e}"))
                    
                    # Handle pipeline migration
                    if render_person['people_pipeline'] and render_person['people_pipeline'] != 'None':
...
'''
    ...
    print(f"✅ Migration script created at: {script_path}")
